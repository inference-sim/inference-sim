...
# =====================================================================
# Config C: Always-admit + severely constrained KV (forces preemptions)
# Same as Config B but with --total-kv-blocks 100 --block-size-in-tokens 16
# This forces KV eviction/preemption with 500 requests.
...
echo "" >&2
echo "--- Config C: Always-admit + constrained KV (total-kv-blocks=100) ---" >&2
...
python3 "$SCRIPT_DIR/analyze.py" \
    ...
    --num-requests 500 \
    --config-label "Config C: Constrained KV (total-kv-blocks=100, stress path)"
