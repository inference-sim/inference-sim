...
// =============================================================================
// H15: Missing LM Head GEMM — Quantify impact of omitted LM head projection
// =============================================================================
//
...
// TestH15_LMHeadWeightFraction computes the LM head weight matrix as a
...
func TestH15_LMHeadWeightFraction(t *testing.T) {
	fmt.Println("=== H15: LM Head Weight Fraction of Total Model Weights ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		t.Logf("H15_WEIGHT_FRACTION model=%s lmhead_bytes=%.0f transformer_bytes=%.0f lmhead_pct=%.4f",
			ms.Name, lmHeadBytes, transformerWeightBytes, lmHeadPct)
	}
	fmt.Println()
}

// TestH15_LMHeadGEMMFLOPs computes the LM head GEMM FLOPs as a fraction of
// total per-step transformer GEMM FLOPs across batch sizes.
func TestH15_LMHeadGEMMFLOPs(t *testing.T) {
	batchSizes := []int{1, 4, 8, 16, 32}

	fmt.Println("=== H15: LM Head GEMM FLOPs as Fraction of Transformer GEMM FLOPs ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		for _, bs := range batchSizes {
			...
			t.Logf("H15_GEMM_FLOPS model=%s bs=%d lmhead_flops=%.0f transformer_flops=%.0f lmhead_pct=%.4f",
...
// TestH15_LMHeadGEMMTime computes the actual time impact of adding the LM head
...
func TestH15_LMHeadGEMMTime(t *testing.T) {
	...
	fmt.Println("=== H15: LM Head GEMM Time Impact (using MFU database) ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		for _, bs := range batchSizes {
			...
			t.Logf("H15_GEMM_TIME model=%s bs=%d transformer_us=%.1f lmhead_us=%.1f lmhead_pct=%.4f",
...
// TestH15_LMHeadMemoryBandwidthImpact computes the memory bandwidth impact
// of adding the LM head weights to the weight loading phase.
func TestH15_LMHeadMemoryBandwidthImpact(t *testing.T) {
	hwCalib := testHardwareCalib()
	peakBW := hwCalib.BwPeakTBs * 1e12
	if hwCalib.BwEfficiencyFactor != 0 {
		peakBW *= hwCalib.BwEfficiencyFactor
	...
	fmt.Println("=== H15: LM Head Memory Bandwidth Impact ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		t.Logf("H15_MEM_BW model=%s transformer_wt_bytes=%.0f lmhead_wt_bytes=%.0f lmhead_pct=%.4f extra_time_us=%.1f",
			ms.Name, transformerWtBytes, lmHeadWtBytes, lmHeadPct, extraTimeUS)
	}
	fmt.Println()
}

// TestH15_DecodeStepTimeComparison computes the full decode step time with and
// without LM head contributions (GEMM time + memory bandwidth), using the
// actual roofline model infrastructure.
func TestH15_DecodeStepTimeComparison(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	...
	fmt.Println("=== H15: Full Decode Step Time With vs Without LM Head ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		for _, sc := range scenarios {
			...
			t.Logf("H15_STEP_TIME model=%s scenario=%s baseline_us=%.1f withlmh_us=%.1f delta_us=%.1f increase_pct=%.4f baseline_roofline_us=%d",
...
// TestH15_ComputeVsMemoryRegime determines whether the LM head addition
// shifts the bottleneck from memory-bound to compute-bound (or vice versa).
func TestH15_ComputeVsMemoryRegime(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	...
	fmt.Println("=== H15: Compute vs Memory Regime Analysis ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		for _, bs := range batchSizes {
			...
			t.Logf("H15_REGIME model=%s bs=%d compute_base_us=%.1f mem_base_us=%.1f regime_base=%s compute_lmh_us=%.1f mem_lmh_us=%.1f regime_lmh=%s",
