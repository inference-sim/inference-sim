...
// =============================================================================
// H17: Decode GEMM Time Underestimate Below MFU Grid Minimum (M < 8)
//
...
// TestH17_GEMMTimeScalingAcrossBatchSizes measures how computeTransformerGEMMTimes
// scales across batch sizes, with particular focus on bs < 8 (below MFU grid minimum).
func TestH17_GEMMTimeScalingAcrossBatchSizes(t *testing.T) {
	...
	fmt.Println("=== H17: GEMM Time Scaling Across Batch Sizes ===")
	fmt.Println()
	...
	for _, ms := range evalSuiteModels() {
		...
		for _, bs := range batchSizes {
			...
			t.Logf("H17_GEMM_SCALING model=%s bs=%d gemm_time_s=%.6e ratio_vs_bs8=%.6f expected_linear=%.6f",
				ms.Name, bs, gemmTime, ratio, expected)
		}

		// === Key check: ratio at bs=1 vs bs=8 ===
		// After the H17 fix (memory-bandwidth floor), bs=1 GEMM time is bounded
		...
		fmt.Printf("H17_KEY_RATIO model=%s ratio_bs1_vs_bs8=%.6f mem_floor_active=%t\n",
			ms.Name, ratio1, ratio1 > 0.5)
	}
}

// TestH17_GEMMTimeScaling_Llama8B is a focused test using just the standard
// Llama-3.1-8B config, for faster execution when bench_data is available.
func TestH17_GEMMTimeScaling_Llama8B(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	hwCalib := testHardwareCalib()
	peakFlops := hwCalib.TFlopsPeak * 1e12
	peakBW := hwCalib.BwPeakTBs * 1e12
	...
	fmt.Println("=== H17: GEMM Time Scaling — Llama-3.1-8B ===")
	...
	ratio1 := gemmTimes[1] / bs8Time
	fmt.Printf("\nH17_POST_FIX_CHECK: ratio_bs1_vs_bs8=%.6f mem_floor_active=%t\n",
		ratio1, ratio1 > 0.5)
...
// TestH17_PerGEMMBreakdown shows the per-projection GEMM time breakdown
// (Q, K, V, O, Gate, Up, Down) for each batch size, demonstrating that
// every individual GEMM has the same scaling behavior.
func TestH17_PerGEMMBreakdown(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	hwCalib := testHardwareCalib()
	peakFlops := hwCalib.TFlopsPeak * 1e12
	peakBW := hwCalib.BwPeakTBs * 1e12
	...
	fmt.Println("=== H17: Per-GEMM Breakdown (Single Layer, Llama-3.1-8B) ===")
	fmt.Println()
	...
	fmt.Println("H17_PER_GEMM_START")
	...
	for _, g := range gemms {
		...
		t.Logf("H17_PER_GEMM name=%s k=%d n=%d ratio_bs1_vs_bs8=%.6f", g.name, g.k, g.n, ratio)
	}

	fmt.Println("H17_PER_GEMM_END")
	fmt.Println()

	// Also show MFU values for each batch size at a representative GEMM shape
	fmt.Println("=== H17: GEMM MFU Values at Different Batch Sizes ===")
	fmt.Printf("%-12s", "GEMM")
	for _, bs := range batchSizes {
	...
	fmt.Println("H17_MFU_VALUES_START")
	for _, g := range gemms {
		fmt.Printf("%-12s", g.name)
		for _, bs := range batchSizes {
	...
	fmt.Println("H17_MFU_VALUES_END")
}

// TestH17_MFUClampingMechanism directly verifies that GetGEMMmfu returns the
...
func TestH17_MFUClampingMechanism(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	...
	fmt.Println("=== H17: MFU Clamping Verification ===")
	fmt.Println()
	...
	fmt.Println("H17_CLAMPING_START")
	...
	for _, shape := range shapes {
		...
		if clampedCount == totalSubgrid {
			t.Logf("H17_CLAMPING %s: ALL sub-grid batch sizes clamped to same MFU (%.6f)", shape.name, mfuBs1)
		} else {
			t.Logf("H17_CLAMPING %s: %d/%d sub-grid batch sizes clamped (interpolation present)", shape.name, clampedCount, totalSubgrid)
		}
	}

	fmt.Println("H17_CLAMPING_END")
}

// TestH17_RegimeAnalysis checks whether small-batch decode steps are
...
func TestH17_RegimeAnalysis(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	hwCalib := testHardwareCalib()
	peakFlops := hwCalib.TFlopsPeak * 1e12
	peakBW := hwCalib.BwPeakTBs * 1e12
	...
	fmt.Println("=== H17: Compute vs Memory Regime at Small Batch Sizes ===")
	...
	for _, bs := range batchSizes {
		...
		t.Logf("H17_REGIME bs=%d gemm_us=%.1f attn_us=%.1f memory_us=%.1f step_us=%d regime=%s",
			bs, gemmTimeS*1e6, attnCoreTimeS*1e6, decodeMemoryS*1e6, stepTimeUS, regime)
	}

	fmt.Println("H17_REGIME_END")
	fmt.Println()

	// Impact analysis: If GEMM at bs=1 had the same time as bs=8 (constant floor),
	// how would that change the step time?
	fmt.Println("=== H17: Impact of Constant GEMM Floor (bs=1 gets bs=8 GEMM time) ===")
	...
	for _, bs := range []int{1, 2, 4} {
		...
		t.Logf("H17_FLOOR_IMPACT bs=%d current_us=%d floor_us=%d delta_us=%d increase_pct=%.2f gemm_delta_us=%.1f",
			bs, currentUS, newStepTimeUS, deltaUS, increasePct, gemmDelta*1e6)
	}
}

// TestH17_MultiModelSummary produces a concise summary table across all eval
// suite models showing the bs=1/bs=8 GEMM time ratio (the key hypothesis metric).
func TestH17_MultiModelSummary(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	hwCalib := testHardwareCalib()
	peakFlops := hwCalib.TFlopsPeak * 1e12
	peakBW := hwCalib.BwPeakTBs * 1e12
	...
	fmt.Println("=== H17: Multi-Model bs=1/bs=8 GEMM Time Ratio Summary (Post-Fix) ===")
	fmt.Println()
	fmt.Printf("%-18s %14s %14s %10s %10s\n",
		"Model", "GEMM_bs1(us)", "GEMM_bs8(us)", "Ratio", "Verdict")
	fmt.Println("------------------------------------------------------------------------------------")

	fmt.Println("H17_SUMMARY_START")
	...
	fmt.Println("H17_SUMMARY_END")
	fmt.Println()
	fmt.Printf("H17_OVERALL: fixed=%d unexpected=%d total=%d\n",
		fixedCount, unexpectedCount, fixedCount+unexpectedCount)

	if fixedCount > 0 && unexpectedCount == 0 {
		fmt.Println("H17_VERDICT=FIXED")
		...
		fmt.Println("H17_VERDICT=UNEXPECTED")
