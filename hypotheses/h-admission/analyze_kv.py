#!/usr/bin/env python3
"""Analyze iter12: compound under KV pressure."""
import json, os, sys

RD = sys.argv[1] if len(sys.argv) > 1 else "hypotheses/h-admission/results"
SEEDS = [42, 123, 7777]
POLS = ["rr", "baseline", "compound", "compound-nokv"]
...
print("=" * 80)
print("Iter 12: Compound under KV pressure (rate=2000, 8 instances)")
print("=" * 80)
for kv in [132139, 5000]:
    print(f"\n--- KV={kv} ---")
    rr_p99 = None
    for pol in POLS:
        vals = []
        for seed in SEEDS:
            r = parse(os.path.join(RD, f"{pol}_kv{kv}_r2000_seed{seed}.json"))
            if r:
                vals.append(r)
        if vals:
            ...
            print(f"  {pol:<20} TTFT p99: {ap:>9.2f}ms  completed: {ac:>5d}{tag}")
        else:
            print(f"  {pol:<20} N/A")
