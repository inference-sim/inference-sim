...
## 9. Round 1 Design Limitations

Round 1 focused narrowly on **per-step time prediction in isolation**. Several aspects of the full BLIS latency pipeline were explicitly out of scope, limiting both the validity and applicability of results.

### 9.1 Step-Time-Only Modeling

All Round 1 experiments predicted raw step duration (`step_time_us`) without modeling the full request lifecycle. In BLIS, observed latency includes:
- **Queueing delay**: Time spent in WaitQ before scheduling (driven by `AlphaCoeffs`)
- **Scheduling overhead**: Batch formation decisions, preemption logic
- **KV cache events**: Allocation latency, eviction/reload cycles (GPU↔CPU offload), transfer delays
- **Inter-step gaps**: Time between consecutive steps (event loop overhead, snapshot refresh)

Round 1 treated step time as the entire signal. In practice, queueing and KV events can dominate E2E latency under load — a model that perfectly predicts step time but ignores these components would still produce large E2E errors.

### 9.2 No BLIS (Go) Integration or Validation

All experiments ran in Python (scikit-learn, XGBoost, scipy). No model was integrated into BLIS's Go `LatencyModel` interface, and no E2E simulation validation was performed. This means:
- The 34% per-step MAPE was never validated against workload-level E2E metrics (TTFT, TPOT, throughput)
- Error cancellation effects (symmetric per-step errors averaging out over a request's lifetime) were not measured
- Interaction between step-time prediction errors and scheduling/routing decisions was not tested
- Feasibility of pure-Go implementation (no Python/ONNX runtime) was not demonstrated

### 9.2.1 Weaker Per-Experiment Baseline

Round 1's blackbox baseline was fitted **per experiment** (16 separate regressions — one per model×workload combination). This is an artificially easy bar to beat: in production, you calibrate once per model deployment, not once per workload type.

A stronger baseline fits one linear regression **per model+TP**, pooling all workloads for that model into a single training set. This forces the 3 coefficients `(beta0, beta1, beta2)` to generalize across general, codegen, roleplay, and reasoning workloads simultaneously — a harder regression problem that better reflects real deployment conditions.

Round 1's claimed "2x improvement" (34% XGBoost vs 70.4% blackbox) was measured against the weaker per-experiment baseline. The improvement margin against a per-model pooled baseline — which must also be validated through BLIS E2E simulation, not just per-step MAPE — may differ substantially.

### 9.3 No Scheduling or Queueing Delay Modeling

BLIS's `LatencyModel` interface has 5 methods, not just `StepTime()`:
- `QueueDelayEstimate()` — estimated wait before first scheduling
- `PreemptionCost()` — cost of evicting/reloading KV blocks
- `StepTime()` — per-step compute duration (**the only method Round 1 modeled**)
- `PrefillChunkSize()` — chunked prefill granularity
- `IsMemoryBound()` — compute vs memory bound indicator

Round 1 only addressed `StepTime()`. The other methods use separate coefficient families (`AlphaCoeffs`) or analytical estimates that were not studied. A complete `LatencyModel` replacement requires data-driven approaches for all five methods.

### 9.4 KV Cache Dynamics Ignored

Step-time prediction was treated as a stateless function of batch composition. In reality:
- KV block allocation/deallocation affects available GPU memory and can trigger eviction
- Tiered KV (GPU+CPU) introduces transfer latencies that add to effective step time
- Preemption decisions depend on KV pressure, creating feedback loops between step time and system state

Round 1's training data captured KV block counts as static features but did not model the dynamic KV lifecycle or its impact on step timing.

### 9.5 Offline Python Analysis vs Online Go Prediction

Round 1 used offline batch prediction (train on historical traces, evaluate on held-out test set). BLIS requires online prediction — `StepTime()` is called on every scheduler iteration with the current batch composition. This creates constraints Round 1 did not address:
- **Prediction latency**: Must be <1ms per call (XGBoost inference overhead in pure Go unknown)
- **Feature availability**: Some features used in Round 1 (e.g., `kv_blocks_free`, `running_depth`) are available differently in the simulator vs in training data
- **Statefulness**: The simulator calls `StepTime()` with `[]*Request` (each carrying `ProgressIndex`), not with pre-computed aggregate features — the feature engineering must happen inside the Go method

---

## 10. Open Questions for Round 2

### 10.1 Can Per-Request KV Features Close the Gap?
...
### 10.2 Can Log-Scale or Normalized Prediction Enable Cross-Model Models?
...
### 10.3 Is Per-Step MAPE the Right Diagnostic?
...
### 10.4 Would More Training Data Help?
...
### 10.5 Can We Leverage the Per-Request Lifecycle Data?
...
### 10.6 What About Simpler Models with Better Features?
...
### 10.7 Can We Handle "General" Workloads Better?
...
### 10.8 Idea 3 Feasibility Without Idea 2
...
6. Round 1 only modeled `StepTime()` in Python — no Go integration, no queueing/scheduling delays, no KV dynamics, no E2E validation
...
7. Modeling all 5 `LatencyModel` methods — not just `StepTime()` — with Go integration from the start
