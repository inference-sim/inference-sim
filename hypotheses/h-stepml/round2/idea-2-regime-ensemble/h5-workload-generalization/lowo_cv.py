#!/usr/bin/env python3
"""Idea 2, H5: Leave-One-Workload-Out (LOWO) cross-validation.

Per-model regime Ridge trained on 2/3 workloads, tested on the held-out workload.
Reports per-step MAPE per (model, workload) fold and runs BLIS E2E.

Usage:
    python3 lowo_cv.py [--output-dir output] [--threshold 256]
"""
import argparse
import json
import os
...
def main():
    ...
    for family in families:
        ...
        for held_out_wl in workloads:
            ...
            # Temporal split within training data for Ridge alpha
            ...
            # -----------------------------------------------------------
            # BLIS E2E on held-out workload experiment
            # -----------------------------------------------------------
            artifact_dir = os.path.join(output_dir, "lowo_artifacts")
            os.makedirs(artifact_dir, exist_ok=True)

            # Use per-model overhead for this family
            ...
            for model_key in family_keys:
                artifact_path = os.path.join(artifact_dir, f"{model_key}_{held_out_wl}_regime.json")
    ...
    print(f"\n  {'='*70}")
    if grand_avg < 25:
        print(f"  HYPOTHESIS SUPPORTED: Grand avg LOWO MAPE = {grand_avg:.1f}% < 25%")
    else:
        print(f"  HYPOTHESIS REFUTED: Grand avg LOWO MAPE = {grand_avg:.1f}% >= 25%")

    if max_mape > 40:
        print(f"  WARNING: ({worst_row['family']}/{worst_row['held_out_workload']}) "
              f"MAPE = {max_mape:.1f}% > 40% single-fold threshold")
    ...
    summary = {
        ...
        "hypothesis_status": "supported" if grand_avg < 25 else "refuted",
        "n_folds": len(overall),
    }
