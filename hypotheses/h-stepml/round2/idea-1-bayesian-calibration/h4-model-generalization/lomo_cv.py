#!/usr/bin/env python3
"""Idea 1, H4: Leave-One-Model-Out (LOMO) cross-validation.

Trains h1's 2-regime piecewise-linear model on 3 of 4 model families,
tests on the held-out model. Reports per-step MAPE per fold and runs
BLIS E2E on held-out experiments.

Usage:
    python3 lomo_cv.py [--output-dir output]
"""
import argparse
import json
import os
...
def main():
    ...
    # -----------------------------------------------------------------------
    print(f"\nStep 3: LOMO 4-fold cross-validation")
    print("=" * 70, flush=True)
    ...
    for fold_idx, held_out_family in enumerate(families):
        ...
        # Train piecewise models on pooled training data
        regime_models = {}
        for regime_name in [REGIME_DECODE_ONLY, REGIME_MIXED_BATCH]:
            ...
            ridge, info = train_regime_model(regime_train, features)
        ...
        if all_preds:
            ...
            overall_r = compute_pearson_r(np.array(all_preds), np.array(all_actuals))
            print(f"    OVERALL: MAPE={overall_mape:.1f}%, r={overall_r:.3f} ({len(all_preds)} steps)")
            lomo_results.append({
                "fold": fold_idx + 1,
                "held_out": held_out_family,
                "regime": "ALL",
        ...
        for held_key in held_out_keys:
            artifact_path = os.path.join(artifact_dir, f"{held_key}_piecewise.json")
        ...
        for dirname in sorted(os.listdir(args.data_root)):
            ...
            artifact_path = os.path.join(artifact_dir, f"{model_key}_piecewise.json")
    ...
    # -----------------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  LOMO CROSS-VALIDATION RESULTS (2-REGIME PIECEWISE)")
    print("=" * 70)
    ...
    for _, row in overall.iterrows():
        r_val = f", r={row['pearson_r']:.3f}" if pd.notna(row.get('pearson_r')) else ""
        print(f"    Fold {int(row['fold'])} (hold out {row['held_out']}): "
              f"MAPE={row['mape']:.1f}%, n={int(row['n_test'])}{r_val}")
    ...
    for regime in [REGIME_DECODE_ONLY, REGIME_MIXED_BATCH]:
    ...
    # Check Mixtral specifically
    mixtral_row = overall[overall["held_out"] == "mixtral"]
    if not mixtral_row.empty:
        mixtral_mape = mixtral_row.iloc[0]["mape"]
        if mixtral_mape > 200:
            print(f"  WARNING: Mixtral MAPE = {mixtral_mape:.1f}% > 200% (MoE transfer failed)")
