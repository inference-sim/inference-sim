...
// =============================================================================
// H30: Decode MFU Interpolation Accuracy
//
...
// h30LoadMFUDatabase loads the real bench_data for H30 tests.
// Skips the test if bench_data is not available.
func h30LoadMFUDatabase(t *testing.T) *MFUDatabase {
	t.Helper()
	mc := testModelConfig() // Llama-3.1-8B: 32-8-128
	benchDataPath := "../bench_data"
	db, err := NewMFUDatabase(mc, benchDataPath, "h100")
	if err != nil {
		t.Skipf("bench_data not available, skipping H30 test: %v", err)
	}
	return db
}

// h30ExtractDecodeGrid extracts the (batchSize, kvLen) -> MFU grid for decode
// attention from the MFU database for the given TP config.
func h30ExtractDecodeGrid(t *testing.T, db *MFUDatabase, tp int) (
...
func TestH30_OnGridAccuracy(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	bsVals, kvVals, grid := h30ExtractDecodeGrid(t, db, tp)
	...
	fmt.Printf("H30_ON_GRID_MAX_ERROR=%.6e\n", maxErr)
	fmt.Printf("H30_ON_GRID_MISMATCHES=%d\n", errCount)
	fmt.Printf("H30_ON_GRID_ZERO_MFU_FALLBACKS=%d\n", zeroMFUFallbackCount)
	fmt.Printf("H30_ON_GRID_TOTAL=%d\n", len(grid))
}
...
func TestH30_OffGridBoundedness(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	bsVals, kvVals, grid := h30ExtractDecodeGrid(t, db, tp)
	...
	fmt.Printf("H30_BOUND_VIOLATIONS=%d\n", violations)
	fmt.Printf("H30_BOUND_TOTAL=%d\n", totalTests)
	fmt.Printf("H30_BOUND_MAX_OVERSHOOT=%.6e\n", maxOvershoot)
}
...
func TestH30_KVAxisMonotonicity(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	bsVals, kvVals, _ := h30ExtractDecodeGrid(t, db, tp)
	...
	fmt.Printf("H30_KV_MONOTONICITY_VIOLATIONS=%d\n", violations)
	fmt.Printf("H30_KV_MONOTONICITY_TOTAL=%d\n", totalTests)
}
...
func TestH30_EvalOperatingPointMFURanges(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	...
	fmt.Printf("H30_EVAL_OPERATING_POINTS:\n")
	...
	fmt.Printf("H30_EVAL_SUSPICIOUS_LOW=%d\n", suspiciouslyLow)
...
func TestH30_InterpolationVsNearestNeighborDelta(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	bsVals, kvVals, grid := h30ExtractDecodeGrid(t, db, tp)
	...
	fmt.Printf("H30_INTERP_VS_NN_TOTAL=%d\n", len(results))
	fmt.Printf("H30_INTERP_VS_NN_MEAN_ABS_DELTA=%.6e\n", meanAbsDelta)
	fmt.Printf("H30_INTERP_VS_NN_MAX_ABS_DELTA=%.6e\n", maxAbsDelta)
	fmt.Printf("H30_INTERP_VS_NN_MEAN_REL_DELTA=%.4f\n", meanRelDelta)
	fmt.Printf("H30_INTERP_VS_NN_MAX_REL_DELTA=%.4f\n", maxRelDelta)
	fmt.Printf("H30_INTERP_VS_NN_COUNT_GT_5PCT=%d\n", count5pct)
...
func TestH30_GridDump(t *testing.T) {
	db := h30LoadMFUDatabase(t)
	tp := 1
	bsVals, kvVals, grid := h30ExtractDecodeGrid(t, db, tp)

	// Print grid header
	fmt.Printf("H30_GRID_DUMP_START\n")
	fmt.Printf("batch_sizes=%v\n", bsVals)
	fmt.Printf("kv_lengths=%v\n", kvVals)

	// Print full grid (on-grid values)
	fmt.Printf("H30_GRID_DATA:\n")
	...
	fmt.Printf("H30_INTERPOLATED_DATA:\n")
	fmt.Printf("%-8s %-8s %-12s\n", "BS", "KVLen", "MFU")
	for bi := 0; bi < len(bsVals)-1; bi++ {
		for ki := 0; ki < len(kvVals)-1; ki++ {
			bs0, bs1 := bsVals[bi], bsVals[bi+1]
...
func TestH30_TPScalingConsistency(t *testing.T) {
	db := h30LoadMFUDatabase(t)
