...
func TestNewKVStore_ZeroBlockSize_Panics(t *testing.T) {
	...
	assert.PanicsWithValue(t,
		...
		func() {
			NewKVStore(SimConfig{KVCacheConfig: KVCacheConfig{TotalKVBlocks: 100, BlockSizeTokens: 0}})
		})
}

func TestNewKVStore_NegativeTotalBlocks_Panics(t *testing.T) {
	assert.Panics(t, func() {
		NewKVStore(SimConfig{KVCacheConfig: KVCacheConfig{TotalKVBlocks: -1, BlockSizeTokens: 16}})
	})
}

func TestNewKVStore_ValidConfig_SingleTier_Succeeds(t *testing.T) {
	// BC-8: Valid config produces a working KVStore
	store := NewKVStore(SimConfig{KVCacheConfig: KVCacheConfig{TotalKVBlocks: 100, BlockSizeTokens: 16}})
...
func TestNewKVStore_ValidConfig_Tiered_Succeeds(t *testing.T) {
	store := NewKVStore(SimConfig{KVCacheConfig: KVCacheConfig{
		TotalKVBlocks:         100,
		BlockSizeTokens:       16,
		KVCPUBlocks:           50,
