...
// TestCluster_OrphanedTimeout_DoesNotInflateSimEndedTime verifies that
// orphaned TimeoutEvents (for already-completed requests) do not inflate
// SimEndedTime in cluster mode. Regression test for
// github.com/inference-sim/inference-sim#1085.
func TestCluster_OrphanedTimeout_DoesNotInflateSimEndedTime(t *testing.T) {
	config := newTestDeploymentConfig(1)

	deadline := int64(300_000_000) // 300s in µs
	reqs := newTestRequests(5)
	for _, r := range reqs {
		r.Deadline = r.ArrivalTime + deadline
	}
	...
	if agg.CompletedRequests != 5 {
		t.Fatalf("CompletedRequests = %d, want 5", agg.CompletedRequests)
	}
	if agg.SimEndedTime > 10_000_000 {
		t.Errorf("SimEndedTime = %d µs (%.1fs), want < 10s — orphaned TimeoutEvents inflated the clock",
			agg.SimEndedTime, float64(agg.SimEndedTime)/1e6)
	}
}

// TestCluster_OrphanedTimeout_SessionFollowUps_StillProcessed verifies that
// the isAllWorkDone() early-exit does NOT break session follow-up processing.
// Follow-up requests generated by OnRequestDone must be fully processed even
// when all initial requests have deadlines.
func TestCluster_OrphanedTimeout_SessionFollowUps_StillProcessed(t *testing.T) {
	...
	cs := NewClusterSimulator(config, reqs, onDone)
	...
	case err := <-done:
		...
		want := numInitial * 2
		if agg.CompletedRequests != want {
			t.Errorf("CompletedRequests = %d, want %d", agg.CompletedRequests, want)
		}
		if agg.SimEndedTime > 10_000_000 {
...
// TestCluster_GenuineTimeout_StillProcessed verifies that requests which
// genuinely time out (deadline fires before completion) are still correctly
// timed out when the early-exit logic is active.
func TestCluster_GenuineTimeout_StillProcessed(t *testing.T) {
	config := newTestDeploymentConfig(1)

	reqs := newTestRequests(3)
	// Give the first request a very short deadline so it genuinely times out.
	reqs[0].Deadline = 1
	for _, r := range reqs[1:] {
		r.Deadline = r.ArrivalTime + 300_000_000
	}
	...
	if agg.TimedOutRequests != 1 {
		t.Errorf("TimedOutRequests = %d, want 1", agg.TimedOutRequests)
	}
	if agg.CompletedRequests != 2 {
		t.Errorf("CompletedRequests = %d, want 2", agg.CompletedRequests)
	}
	if agg.SimEndedTime > 10_000_000 {
		t.Errorf("SimEndedTime = %d µs (%.1fs), want < 10s", agg.SimEndedTime, float64(agg.SimEndedTime)/1e6)
	}
...
// TestCluster_OrphanedTimeout_Conservation verifies INV-1 (request conservation)
// holds with the isAllWorkDone() early-exit.
func TestCluster_OrphanedTimeout_Conservation(t *testing.T) {
	config := newTestDeploymentConfig(1)
	...
	accounted := int64(agg.CompletedRequests) +
		int64(agg.StillQueued) +
		int64(agg.StillRunning) +
		int64(agg.DroppedUnservable) +
		int64(agg.TimedOutRequests) +
		int64(cs.RoutingRejections()) +
		int64(cs.GatewayQueueDepth()) +
		int64(cs.GatewayQueueShed())

	if injected != accounted {
		t.Errorf("INV-1 violation: injected=%d, accounted=%d (completed=%d, queued=%d, running=%d, dropped=%d, timed_out=%d)",
			injected, accounted, agg.CompletedRequests, agg.StillQueued, agg.StillRunning, agg.DroppedUnservable, agg.TimedOutRequests)
	}
}
