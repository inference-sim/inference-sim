...
// h33BatchSpec defines a heterogeneous decode batch for the experiment.
type h33BatchSpec struct {
	name   string
	kvLens []int64
}

// h33Scenarios returns the decode batch compositions for the experiment.
// These span a range of KV heterogeneity from mild (2x range) to extreme (64x range).
func h33Scenarios() []h33BatchSpec {
	return []h33BatchSpec{
...
// TestH33_MFUShapeMismatch is the core experiment. For each batch, it computes
...
func TestH33_MFUShapeMismatch(t *testing.T) {
	...
	scenarios := h33Scenarios()

	// Print structured output for analyze.py
	fmt.Println("H33_RESULTS_START")
	...
	for _, sc := range scenarios {
		...
		kvRange := float64(maxKV) / float64(sc.kvLens[0]) // assumes sorted or first is min
		...
		t.Logf("H33 scenario=%s bs=%d maxKV=%d meanKV=%.0f range=%.0fx currentTime=%.9f perReqTime=%.9f ratio=%.6f",
			sc.name, bs, maxKV, meanKV, kvRange, currentTimeS, perReqTimeS, ratio)
	}

	fmt.Println("H33_RESULTS_END")

	// Summary
	fmt.Println()
	fmt.Printf("H33_SUMMARY: %d/%d scenarios within 5%% threshold (ratio in [0.95, 1.05])\n",
		withinThreshold, totalScenarios)
}

// TestH33_MFUMonotonicity verifies the underlying assumption of the hypothesis:
...
func TestH33_MFUMonotonicity(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	tp := 1
	...
	fmt.Println("H33_MFU_MONOTONICITY_START")
	fmt.Printf("%-8s", "KVLen")
	...
	fmt.Println("H33_MFU_MONOTONICITY_END")

	// Check monotonicity along KV axis for each batch size
	fmt.Println()
	fmt.Println("H33_MONOTONICITY_CHECK:")
...
// TestH33_PerRequestMFUProfile shows the MFU value at each request's KV length
...
func TestH33_PerRequestMFUProfile(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	tp := 1

	// Use a few representative batches
	batches := []h33BatchSpec{
	...
	fmt.Println("H33_MFU_PROFILE_START")
	...
	fmt.Println("H33_MFU_PROFILE_END")
}

// TestH33_StepTimeImpact computes the full decode step time (not just attention)
...
func TestH33_StepTimeImpact(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	mc := testModelConfig()
	hc := testHardwareCalib()
	tp := 1
	...
	fmt.Println("H33_STEP_TIME_START")
	...
	for _, sc := range scenarios {
		...
		t.Logf("H33_STEP scenario=%s baseline_us=%d adjusted_us=%d ratio=%.6f attn_frac=%.2f%%",
			sc.name, baselineUS, adjustedStepUS, stepRatio, attnFraction)
	}

	fmt.Println("H33_STEP_TIME_END")
}

// TestH33_KVRangeVsRatio sweeps KV range factor systematically to show the
...
func TestH33_KVRangeVsRatio(t *testing.T) {
	mfuDB := loadTestMFUDatabase(t)
	mc := testModelConfig()
	hc := testHardwareCalib()
	...
	fmt.Println("H33_RANGE_SWEEP_START")
	...
	fmt.Println("H33_RANGE_SWEEP_END")
}
