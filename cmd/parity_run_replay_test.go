...
// runSpecToTraceFilesSpecDecode is runSpecToTraceFiles with speculative-decoding
// flags threaded through both run and replay (#1528). It exercises the INV-13 path
// where a model-level config is re-supplied to replay via identical flags rather
// than persisted in the trace.
func runSpecToTraceFilesSpecDecode(t *testing.T, specYAML string, seedVal, horizon int64, k int, acc float64) (headerFile, dataFile string) {
	t.Helper()
	tmpDir := t.TempDir()
	tracePrefix := filepath.Join(tmpDir, "trace")
	specPath := filepath.Join(tmpDir, "workload.yaml")
	...
	args := []string{
		...
		"--trace-output", tracePrefix,
		"--num-speculative-tokens", strconv.Itoa(k),
		"--speculative-acceptance-rate", strconv.FormatFloat(acc, 'f', -1, 64),
	}
	if err := testCmd.ParseFlags(args); err != nil {
		t.Fatalf("ParseFlags: %v", err)
	}
...
// replaySpecTraceSpecDecode is replaySpecTrace with speculative-decoding flags.
func replaySpecTraceSpecDecode(t *testing.T, traceHeaderFile, traceDataFile string, k int, acc float64) []workload.SimResult {
	t.Helper()
	tmpDir := t.TempDir()
	resultsFile := filepath.Join(tmpDir, "results.json")
	mcFolder, hwPath, defaultsPath := setupTrainedPhysicsTestFixturesWithDefaults(t)
	...
	origReplayTraceOut, origCacheSignalDelay, origFlowControlEnabled := replayTraceOutput, cacheSignalDelay, flowControlEnabled
	origK, origAcc, origMethod := numSpeculativeTokens, speculativeAcceptance, speculativeMethod
	defer func() {
		...
		replayTraceOutput, cacheSignalDelay, flowControlEnabled = origReplayTraceOut, origCacheSignalDelay, origFlowControlEnabled
		numSpeculativeTokens, speculativeAcceptance, speculativeMethod = origK, origAcc, origMethod
	}()
	...
	if err := testCmd.ParseFlags([]string{
		...
		"--model-config-folder", mcFolder, "--hardware-config", hwPath,
		"--trace-header", traceHeaderFile, "--trace-data", traceDataFile,
		"--results-path", resultsFile, "--num-instances", "1",
		"--horizon", "120000000", "--defaults-filepath", defaultsPath,
		"--num-speculative-tokens", strconv.Itoa(k),
		"--speculative-acceptance-rate", strconv.FormatFloat(acc, 'f', -1, 64),
	}); err != nil {
		t.Fatalf("ParseFlags: %v", err)
...
// TestParity_RunReplay_SpecDecode pins INV-13 for speculative decoding (#1528, BC-7):
// a trace exported from `blis run --num-speculative-tokens K` and replayed with the
// same flags produces identical per-request TTFT/E2E. Because the config is
// model-level and re-supplied to replay (not persisted in the trace) — exactly like
// --tp / --max-model-len — this confirms the shared-flag INV-13 approach holds.
//
// NOTE: Do NOT use t.Parallel() — mutates package-level vars.
func TestParity_RunReplay_SpecDecode(t *testing.T) {
	const seed int64 = 20260808
	const k = 3
	const acc = 0.6
	shape := paritySpecShapes()[0] // chatbot

	hdr, data := runSpecToTraceFilesSpecDecode(t, shape.yaml, seed, shape.horizon, k, acc)
	runResults := replaySpecTraceSpecDecode(t, hdr, data, k, acc)
	if len(runResults) == 0 {
		t.Fatal("INV-13 spec-decode: replay produced no completed requests")
	}
	// Replay a second time: identical (determinism + parity).
	runResults2 := replaySpecTraceSpecDecode(t, hdr, data, k, acc)
	if len(runResults) != len(runResults2) {
		t.Fatalf("INV-13 spec-decode: replay count differs %d vs %d", len(runResults), len(runResults2))
	}
	ttft1, e2e1 := simResultMaps(runResults)
	ttft2, e2e2 := simResultMaps(runResults2)
	for id, want := range ttft1 {
		if ttft2[id] != want {
			t.Errorf("INV-13 spec-decode: request %d TTFT %f vs %f", id, want, ttft2[id])
		}
		if e2e1[id] != e2e2[id] {
			t.Errorf("INV-13 spec-decode: request %d E2E %f vs %f", id, e2e1[id], e2e2[id])
		}
	}
}
