...
func resolveLatencyConfig(cmd *cobra.Command) latencyResolution {
	...
	// --latency-model evolved: experimental physics-informed roofline from training branch.
	...
	if backend == "blackbox" && allZeros(alpha) && allZeros(beta) {
		logrus.Fatalf("No trained coefficients found for model=%s, GPU=%s, TP=%d. "+
			"Provide --alpha-coeffs/--beta-coeffs, use --latency-model roofline, crossmodel, trained-roofline, trained-physics, or evolved",
	...
	if backend == "roofline" || backend == "crossmodel" || backend == "trained-roofline" || backend == "trained-physics" || backend == "evolved" {
...
func registerSimConfigFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&latencyModelBackend, "latency-model", "roofline", "Latency model backend: roofline (default), blackbox, crossmodel, trained-roofline, trained-physics, evolved")
...
var runCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if lr.Backend == "roofline" || lr.Backend == "crossmodel" || lr.Backend == "trained-roofline" || lr.Backend == "trained-physics" || lr.Backend == "evolved" {
