...
| **VI. Configuration Discipline** | ✅ PASS | New fields (`ModelAutoscalerIntervalUs`, `HPAScrapeDelay`, `ScaleUpCooldownUs`, `ScaleDownCooldownUs`) added to `DeploymentConfig` (R16), not `SimConfig`. `CostPerHour` added to `NodePoolConfig`. `DelaySpec` (existing) used for `HPAScrapeDelay`. |
