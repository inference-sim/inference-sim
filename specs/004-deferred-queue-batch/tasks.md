...
- [X] T002 [US1] Write `TestDeferredQueue_BatchDeferredWhenBusy` in `sim/cluster/cluster_deferred_test.go` — GIVEN a 2-instance cluster processing standard requests (busy), WHEN a batch request arrives, THEN it is NOT counted as rejected (`cs.RejectedRequests()` unchanged) and `cs.DeferredQueueLen()` == 1 after `Run()`. Verify with `go test ./sim/cluster/... -run TestDeferredQueue_BatchDeferredWhenBusy` → must FAIL with compile or assertion error before impl
...
- [X] T004 [US1] Write `TestDeferredQueue_DeferredPromotedAfterIdle` in `sim/cluster/cluster_deferred_test.go` — GIVEN 5 deferred batch requests and 10 standard requests that complete first, WHEN `Run()` completes, THEN all 15 requests are accounted for as completed + deferred_horizon_interrupted == 0 (all batch requests eventually processed). Verify FAIL before impl
...
- [X] T006 [US3] Write `TestDeferredQueue_INV1_Conservation` in `sim/cluster/cluster_deferred_test.go` — GIVEN a simulation with 20 batch requests and a finite horizon where some batch requests remain deferred at end, WHEN `Run()` completes and metrics are collected, THEN: `injected == completed + still_running + still_queued + shed + dropped + DeferredQueueLen()`. Use `newTestDeploymentConfig(1)` with horizon to force deferred at end. Verify FAIL before impl
