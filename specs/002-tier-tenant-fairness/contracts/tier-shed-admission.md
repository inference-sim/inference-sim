...
### Postconditions
- Returns `(true, "")` when `SLOTierPriority(req.SLOClass)` ≥ `MinAdmitPriority` OR max effective load ≤ `OverloadThreshold`
- Returns `(false, reason)` when `SLOTierPriority(req.SLOClass)` < `MinAdmitPriority` AND overloaded
...
**Zero-value footgun**: `MinAdmitPriority=0` means "admit all tiers under overload" — equivalent to `AlwaysAdmit`. This is technically correct but almost certainly not the intended configuration. The constructor (or the YAML unmarshaller) should default `MinAdmitPriority` to `3` (Standard) when the field is absent, so callers must explicitly opt into lower thresholds.
