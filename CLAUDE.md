...
## Recent Changes
- Preemption policy (#1086): `--preemption-policy fcfs|priority` flag (default: `fcfs`). In `priority` mode, preemption evicts the least-urgent running request based on SLO tier priority (background → sheddable → batch → standard → critical), matching vLLM's `--scheduling-policy priority` preemption path. Phase 1 index adjustment (`reqIndex -= adj`) prevents element skipping after non-tail eviction (analog of vLLM `scheduler.py:853` `req_index -= 1`). Priority convention: BLIS `min(SLOPriority)` = vLLM `max(priority)` due to inverted conventions. Configurable via CLI flag or policy bundle YAML (`preemption: { policy: priority }`). Both `blis run` and `blis replay` support the flag.
