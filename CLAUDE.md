...
**Multi-node TP placement (#1529, PR A)**: A model instance can occupy WHOLE NODES across a single node pool, enabling multi-node tensor parallelism (e.g. GLM-5.2 at TP=16 on 2×8 H100, which no single node can host). `PlacementManager.PlaceInstance` runs two global passes over the matching pools: **Pass 1** is the original single-node first-fit across all pools (kept verbatim — single-node placement is byte-identical, INV-6); **Pass 2** (whole-node cross-node placement within one pool) is reached ONLY when no single Ready node in any matching pool can satisfy `tpDegree`, so single-node placement is strictly preferred to spanning anywhere. Pass 2 is modeled as **whole-node occupancy**: a pool is eligible only when `tpDegree > gpus_per_node` (spanning is physically necessary) AND `tpDegree % gpus_per_node == 0` (ranks divide evenly), and the instance takes exactly `tpDegree/gpus_per_node` **fully-free** Ready nodes, each contributing all its GPUs — the only physically-realizable multi-node TP shape (uniform per-node rank count). The **fragmentation case** (`tpDegree ≤ gpus_per_node` but no single node momentarily has room) is deliberately NOT spanned — packing an odd remainder across nodes (e.g. 2+1 for TP=3) would fabricate an asymmetric TP group that cannot exist; such an instance stays pending exactly as before this feature. Select-then-commit (R5): a pool that cannot supply enough whole free nodes mutates nothing. Node span is derived from the placed GPU set via `PlacementManager.distinctNodesForGPUs` (backed by a `gpusByID` index; no GPU-ID string parsing; a GPU ID missing from the index is logged as an R4 invariant violation, never silently dropped). A spanning instance's per-instance `CostPerHour` = distinct-nodes-spanned × pool `cost_per_hour` (`InstanceCostPerHour`, applied at all three placement sites; single-node is 1× — unchanged; an empty GPU set logs and returns 0 rather than a plausible 1×). `ReleaseInstance` fires the drain-completion callback for EVERY node a spanning instance freed (the pre-#1529 code checked only the last node visited). **Cross-node latency is deliberately deferred to #1530**: the trained-physics TP all-reduce term (β₄) assumes intra-node NVLink and is placement-blind, so a spanning instance's all-reduce is priced optimistically; the first time any instance spans, a one-time `logrus.Warn` (stderr, never stdout — INV-6) says so and names #1530. `blis replay`/`observe` reject node pools, so this is `blis run` only (INV-13 parity N/A). Scope: multi-node TP placement only; pipeline parallelism (the other half of #1529) is a separate design-doc-first PR (#1535). Cross-*pool* spanning (one TP group over mixed GPU types) is out of scope. Known limits (both tracked as #1536): (1) the autoscaler's per-variant `CostPerReplica` (keyed on `{GPUType, TPDegree}`) still takes the first replica's cost, so a mixed span/non-span population within one variant is a single-value approximation; (2) the autoscaler's scale-up headroom (`GreedyEngine.Optimize`) gates on free GPU *count*, not free whole *nodes*, so for a spanning variant it can propose a scale-up that `PlaceInstance` then rejects (e.g. 16 free GPUs spread as one whole node + two half-free nodes cannot host a whole-node TP=16) — the failed scale-up is logged and returned harmlessly, but may repeat each autoscaler tick.
