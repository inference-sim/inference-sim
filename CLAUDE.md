...
## Recent Changes
- Per-tenant fair dispatch — FairnessPolicy interface (#1192): Pluggable `FairnessPolicy` interface for within-band flow selection in `GatewayQueue`. Two implementations: `GlobalStrictPolicy` (default, picks earliest seqID — backward compatible) and `RoundRobinPolicy` (cursor-based tenant cycling, per-band cursor isolation). New `--fairness-policy` flag (`global-strict`, `round-robin`). Applies to priority dispatch (`dequeuePriority`) and gated dispatch (`DequeueGated`); FIFO mode unaffected (flat global scan). Closes #1192, part of #899.
- Priority convention alignment (#1216): Instance-level priority now uses vLLM convention (lower=urgent). Pre-processor at `Simulator.EnqueueRequest()` and `EnqueueDecodeSubRequest()` sets `Request.Priority = float64(sloMap.InvertForVLLM(SLOClass))` — mirrors llm-d→vLLM dispatch boundary. `PriorityFCFSScheduler.OrderQueue` sorts ascending (`<`). `selectPriorityVictim` reads `Request.Priority` directly and uses `max()` (matches vLLM `scheduler.py:1086`). `PriorityPolicy.Compute()` step-loop removed (static priority model). `SLOPriorityMap` extracted to `sim/slo_priority.go`. `NewBatchFormation` is now 1-arg (sloMap dropped). `--priority-policy slo-based/inverted-slo` are no-ops (per-step aging removed; warning emitted). Cluster layer (admission, gateway queue) unchanged — llm-d convention preserved.
...
- SLO priority override threading (#1170): `SLOPriorityOverrides` field moved from `DeploymentConfig` to `SimConfig`, enabling policy bundle `slo_priorities` to reach priority preemption. **Updated by #1216**: `SLOPriorityOverrides` now flows into `Simulator.sloMap` (not `VLLMBatchFormation`); preemption reads `Request.Priority` set by the pre-processor at enqueue. `NewBatchFormation` is now 1-arg (sloMap parameter removed).
...
- Priority preemption mode (#1169): `--preemption-policy priority` evicts the least-urgent running request under KV pressure. **Updated by #1216**: `selectPriorityVictim()` now reads `Request.Priority` directly (vLLM convention: `max(Priority)` with `max(ArrivalTime)` tiebreak — direct parity with vLLM `scheduler.py:1086`). Phase 1 index adjustment (`reqIndex -= adj`) unchanged. `NewBatchFormation` is now 1-arg (sloMap removed; priority flows via `Simulator.sloMap` pre-processor).
