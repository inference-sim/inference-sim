...
## Recent Changes
- QueueingModelAnalyzer (#1200): `QueueingModelAnalyzer` implements M/M/1/K-SD queueing model parameter estimation for the autoscaler `Analyzer` interface. Three-phase estimation: (1) `InitEstimator` (Nelder-Mead, requires `init_obs` observations), (2) `SlidingWindowEstimator` (re-fit every cycle, `use_sliding: true`), (3) EKF (default). SLO target resolution in priority order: explicit `qm_config.slo_targets` (ms) → derived from fitted α/β/γ latency parameters × `slo_multiplier` → observed per-replica TTFT/ITL × fallback headroom. Configured via `qm_config` in policy bundle YAML; zero value safe — all numeric defaults applied by `NewQueueingModelAnalyzer`. Wiring pending issue #954 (AnalyzerType selector); requires latency-populating collector (#1198) for live signals (`DefaultCollector` produces zero TTFT/ITL/AvgInTokens/AvgOutTokens).
