...
## Recent Changes
- fix(admission): gate deferred queue behind config (#1008): `EnableDeferredQueue` bool on `DeploymentConfig` (default `false`). When false, batch/background requests proceed directly to admission — no silent deferral. `--admission-policy tier-shed` auto-enables deferral for backward compatibility. New CLI flag: `--enable-deferred-queue`. Policy bundle support via `admission.enable_deferred_queue` YAML field. Fixes semantic mismatch where `always-admit` silently deferred batch/background traffic.
