...
- Remove deferred-queue pre-admission intercept (#1012/#1016): Batch/background requests no longer intercepted before admission — they flow through normal admission pipeline. `DeferredQueue` retained for `NodeReadyEvent` lifecycle only. Deferred-queue depth removed from INV-1 conservation equation.
...
- Event-driven cache signal propagation (#1029): `--cache-event-delay` (renamed from `--cache-signal-delay`) replaces global periodic poll with per-instance event-driven refresh. After each step that allocates KV blocks, a `CacheEventArrivalEvent` fires at `now + CacheEventDelay` to refresh that instance's stale snapshot via `StaleCacheIndex.RefreshInstance()`. Detection via `AllocationEpoch()` monotonic counter on `KVStore` interface (respects sim/kv layering). Default changed from 2s to 50ms (50,000 µs). **Breaking change:** Existing scripts using `--cache-signal-delay` without an explicit value will now use 50ms delay instead of 2s. To restore old behavior, pass `--cache-event-delay 2000000` explicitly. Old flag `--cache-signal-delay` kept as deprecated alias. INV-7 table updated.
