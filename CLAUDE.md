...
## Recent Changes
- blis run --timeout flag (#1127): `--timeout` (seconds, default `-1` = disabled) overrides per-request deadline for all synthesized clients. Positive value sets a deadline; negative disables it (explicit `*0` suppresses the session-client 300s default in `computeDeadline`); `0` is rejected as invalid — consistent with `blis observe` which also rejects `<= 0`. File-loaded `--workload-spec` clients are only overridden when `--timeout` is explicitly set. Warns on stderr when any requests time out. `blis observe` has its own `--timeout` (HTTP client timeout, default 300s, configurable via #1118; rejects `<= 0`). `blis replay` has no HTTP client and no `--timeout` flag.
