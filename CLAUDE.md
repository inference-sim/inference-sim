...
**Offload cache hits reduce billed prefill (`sim/kv.OffloadCache` + `sim.ReloadReportingKVStore`, #1699)**: the fix for a billing bug where a CPU/secondary offload hit moved the reported `cache_hit_rate` but left **every** latency metric byte-identical (changing `cpu_bytes_to_use` swung hit-rate 0.077→0.367 with TTFT frozen). Root cause: `FormBatch` Phase 2 computes a new request's billed prefill work (`numNewTokens`, the sole `StepTime` input) from the **GPU-only** `GetCachedBlocks` **before** the CPU→GPU reload runs *inside* `AllocateKVBlocks` — so a reload that lands prefix blocks on the GPU (and bumps `CacheHits`) never reduced `NumNewTokens`, and a genuine hit was billed as a full recompute. This is BLIS's missing analogue of vLLM's `num_new_tokens = num_tokens − (num_local_computed + num_external_computed)` (`scheduler.py`): the external/offload hit must shrink the tokens sent through the prefill forward pass. **Mechanism:** a new optional `sim.ReloadReportingKVStore` interface (`ReloadedPrefixEnd(reqID) (newStart, ok)`), implemented by **both offload stores** — `OffloadCache` (the #1590 chain) and the legacy `TieredKVCache` (`--kv-cpu-blocks`), which is the structural twin with the identical bug — but NOT by the non-offload single-tier `KVCacheState` (same inert-for-others type-assert pattern as the `DeferrableKVStore` step-boundary deferral above, #1591). `OffloadCache.recordReloadedPrefix` records the post-reload GPU-cached prefix boundary (`newStart`) during `allocateThroughChain`, gated to **genuinely-new prefill admissions** (`!running && !IsDecodeSubRequest`, the same class `FormBatch` re-bills — a running continuation bills incrementally against `ProgressIndex` and a PD decode sub-request is admitted via the decode branch) and written **only on the admit-success paths** (a failed tail alloc leaves the request in the WaitQ where the boundary would never be consumed, so recording it there would leak a stale entry). `FormBatch` type-asserts the interface and re-bills `billedTokens = endIndex − min(newStart, endIndex)` — the non-reloaded tail **within the committed chunk**. **`ComputedTokens` (progress) is unchanged**: the reload changes how much *compute* a chunk costs, NOT how far the request advances — the whole `[startIndex, endIndex)` range was committed, so a reload extending past a capped chunk (`newStart > endIndex`) bills 0 without over-crediting progress to `InputLen`. The read is **one-shot** (consumed on lookup, so a later step cannot re-bill a stale boundary); `ReleaseKVBlocks` and `ClearDeferred` also clear the entry so a preempt→re-prefill or timeout/eviction cannot carry one forward. **Covers BOTH offload paths** (they funnel through `allocateThroughChain`): the CPU-only synchronous reload and the resolved-secondary-deferral (H3) admit. **The legacy `--kv-cpu-blocks` `TieredKVCache` is fixed identically** — it is the structural twin (`reloadPrefixFromCPU` → `newStart` → same `newStart > startIndex`/`>= endIndex` commit), so it records the boundary at its own two commit-success points under the same `!running && !IsDecodeSubRequest` gate. **Scope:** prefill-shrink only — the CPU-only path stays a same-step optimistic reload (no explicit transfer latency), while the secondary path keeps its existing H3 one-step load cost; routing CPU-tier hits through a one-step load (vLLM `WAITING_FOR_REMOTE_KVS`) is a documented follow-up. **Known limitation (#1706): the credit is capped at ONE chunk.** `billedTokens` shrinks only within the committed `[startIndex, endIndex)` chunk; a reload extending the prefix past `endIndex` leaves `ComputedTokens` at `endIndex`, so the reloaded remainder is re-billed as full recompute by later Phase-1 continuations (which bill `InputLen − ProgressIndex` and consult no cache). vLLM instead advances `num_computed_tokens = local + external` *before* the `long_prefill_token_threshold`/budget cap, crediting the whole reloaded prefix. So under **chunked prefill** BLIS realizes only the single-chunk share (≈ `chunk/prompt`): ~99% when the prompt fits one chunk, but ~2% for long-context offload workloads (e.g. weka Kimi ~110K ISL at the 2048-token default). #1699's 1280-token repro sits entirely in the uncapped (one-chunk) regime, which is why its effect table shows the full credit. Crediting to `newStart` requires extending the block commit past `endIndex` (#1706). **INV-6:** the non-offload single-tier `KVCacheState` doesn't implement the interface ⇒ `reloadReporter` is nil ⇒ the re-bill block is dead code ⇒ a run without offload is byte-identical. (Both offload stores DO implement it — turning it on is the intended, output-changing case.) **INV-13:** the re-bill is a pure function of deterministic allocation state (no RNG/wall-clock) ⇒ replay reproduces it. `ReloadsPerformed()` exposes the existing `reloadCount` (#1586 diagnostic family) — the count of blocks billed as a hit rather than a recompute. Files: `sim/kv_store.go`, `sim/kv/offload_chain.go`, `sim/kv/offload_deferral.go`, `sim/kv/tiered.go` (legacy path), `sim/batch_formation.go` (+ `sim/kv/offload_chain_test.go`, `sim/kv/offload_deferral_test.go`, `sim/kv/tiered_test.go`, `sim/batch_formation_reload_test.go`; e2e `sim/cluster/offload_deferral_e2e_test.go`).
