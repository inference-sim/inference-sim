...
## Recent Changes
- fix(observe): double-invocation protection (#1047): Added diagnostic logging (invocation ID at entry/exit) and `sync.Once` guards around Export/ExportITL calls to prevent file corruption from double-invocation. If `runObserve` is called twice in the same process, exports run exactly once. Diagnostic logs capture evidence for root cause analysis.
