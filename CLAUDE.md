...
## Recent Changes
- blis run --timeout flag (#1127): `--timeout` (seconds, default 0 = no timeout) overrides per-request deadline for all synthesized clients before generation. Explicit `0` suppresses the session-client 300s default in `computeDeadline`. File-loaded `--workload-spec` clients are only overridden when `--timeout` is explicitly set. Warns on stderr when any requests time out (`%d of %d requests timed out (%ds deadline)`), naming `tokens_per_sec` as the potentially inflated metric. `blis observe` and `blis replay` retain their existing 300s HTTP-client timeout.
