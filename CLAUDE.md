...
- **INV-1 Request conservation**: `injected_requests == completed_requests + still_queued + still_running + in_transit + dropped_unservable` at simulation end (`in_transit` is 0 in mixed mode). Full pipeline: `num_requests == injected_requests + rejected_requests`.
