...
# NS-6 (#1733) + S4 (#1731) + #1774 — three rules every `blis run`/`blis replay` example
# below obeys:
#  1. The CATALOG MUST BE LOCATED, by --catalog <path> or the BLIS_CATALOG environment
#     variable, with NO default, NO search path and NO remote fetch (--catalog wins when
#     both are set, announced on stderr). A run with neither is REFUSED naming both forms.
#     Both name the catalog CLONE ROOT (#1774), not its models/ directory. A RELATIVE value
#     is resolved against the process working directory; an ABSOLUTE value is used as given.
#     The retired --model-config-folder is gone: point --catalog at a scratch directory to
#     use your own config, so there is exactly one way to supply a model config.
#     The examples below OMIT the flag — export it once from the repository root:
#         export BLIS_CATALOG=$PWD/model_configs
#     (that bundled tree is FLAT, and keeps resolving via the #1774 transition fallback
#     until #1771 removes both it and the fallback.)
#     `blis observe` takes neither form (it resolves no model config).
#  2. A model runs IFF it is in that catalog. The model's config.json is read from
#     <catalog>/models/<short-name>/config.json, with a transition fallback to the flat
#     <catalog>/<short-name>/config.json (#1774). A model absent from BOTH layouts is
#     REFUSED naming every path looked at and the canonical models/ path its entry belongs
#     at. Only ABSENCE falls through: a malformed models/ entry is reported, never silently
#     replaced by a flat one. No run-time HuggingFace fetch, and no run/replay/observe
#     creates or modifies a catalog file. Add a model by committing its config.json.
#  3. --hardware and --tp are REQUIRED, on both `blis run` and `blis replay`. They are no
...
# multi-node only). The size-independent cross-node latency term is n_steps·α_hop·S:
# n_steps is the analytic hop count of the placed node span (ring 2·(nodes−1), all-to-all
# (nodes−1)); α_hop is the per-fabric HardwareCalib.InterNodeHopLatencyUs (0/uncalibrated
# in the bundled config ⇒ inert); S is the deployment serialization multiplier.
# --comm-serialization-factor S (default 1.0 = graphs-on/overlap, inert & byte-identical,
#   INV-6). >1 models enforce-eager / no comm-compute overlap. Must be >= 1; multiplies an
#   EXISTING cross-node latency term (spans nodes AND α_hop>0), never creates cost on its own.
...
- **INV-1 Request conservation**: the canonical form is the twelve-term **cluster** equation `injected_requests == completed_requests + still_queued + still_running + dropped_unservable + timed_out + routing_rejections + gateway_queue_depth + gateway_queue_shed + gateway_queue_rejected + gateway_evicted + gateway_expired + encode_routing_rejections` at simulation end. A **single-instance** run has no cluster router, gateway queue or encode pool, so the last seven terms are zero and it reduces to five terms — assert the five-term form ONLY for single-instance, never at cluster level (#1721). Full pipeline: `num_requests == injected_requests + rejected_requests`. `encode_routing_rejections` (GAP-4, #1264) is always zero when `--encode-instances 0`.
- **INV-2 Request lifecycle**: Requests transition queued → running → completed; not completed before horizon remain in current state. Contrast INV-14 (#1772): the *instance* lifecycle is the same class of invariant with the opposite protection profile — a legal-transition table and a panicking choke point, where INV-2 has an entry and no enforcement.
- **INV-3 Clock monotonicity**: Simulation clock never decreases; every **processed** event's timestamp >= the previous processed event's. Carve-out: restoring an optimistic advance after a lazily-cancelled event, where no event was processed (`sim/cluster/cluster.go`, the `prevClusterClock` restore) — so assert on processed event timestamps, never on a raw clock field. Distinct from INV-14's sibling INV-15 (#1772), which constrains the arrival stream the cluster is *given* rather than the clock it advances. See `docs/contributing/standards/invariants.md`.
...
- **INV-14 Instance lifecycle transitions**: An instance's `State` changes only along an edge of `validInstanceTransitions` (`sim/cluster/instance.go`); `TransitionTo` — the only writer of `State` outside construction — panics otherwise. The edge set is monotone (Scheduling < Loading < WarmingUp < Active < Draining < Terminated, never backward). Clause 2, the **seeding carve-out**: when `State == ""` (lifecycle tracking disabled) the *first* transition is accepted unvalidated to seed the state; every later one is validated. See `docs/contributing/standards/invariants.md`.
- **INV-15 Arrival ordering into the cluster**: The fresh-arrival stream handed to `ClusterSimulator`'s arrival hook is non-decreasing in arrival time; `SetArrivalHook(nil)` resets the floor. It is a **precondition on an input**, not INV-3 or INV-6 (the IDs the guard cited before #1772) — the repair is in the arrival source, not the clock or output ordering. `--trace-output` relies on it to emit records already in arrival order without a downstream sort. Scope: fresh arrivals only (`REDIRECT` re-injections never fire the hook). See `docs/contributing/standards/invariants.md`.
- **INV-16 Gateway queue counter consistency**: `GatewayQueue.totalLen` always equals the entries held across all bands and flows, and each `priorityBand.totalLen` equals its own band's; a dequeue that finds the counter positive must return an entry (three panic sites). Same lockstep-counter shape as INV-4 one subsystem over, but over a two-level structure. The failure mode is a **stuck queue**, and INV-1 conservation does not catch it. See `docs/contributing/standards/invariants.md`.
- **INV-17 Shed victim is the flow tail**: Within a flow the shed victim is always the tail (highest `seqID`), so `removeEntryByIndex` is a truncation and the head `Dequeue` reads is preserved. Tail-ness is a *consequence* of `findGlobalShedVictim`'s tie-break, so a victim-selection change breaks it from a distance — the same index-drift class INV-12 covers on the batch-formation side. See `docs/contributing/standards/invariants.md`.
- **INV-18 Prefix-cache LRU structural consistency**: In every `lruBlockCache` the intrusive linked list and the `lookup` map describe the same block set; `tail == nil` iff `len(lookup) == 0`, so eviction at capacity can never fail. Silent until it isn't: a diverged list still answers `MatchLength`, just with fewer hits, so routing degrades and reads as a workload effect. See `docs/contributing/standards/invariants.md`.
- **INV-19 Scale decisions carry a non-zero delta**: Every `ScaleDecision` an `Engine.Optimize` emits has `Delta != 0`; a zero delta is **warned and skipped, not fatal** — deliberately, because a zero delta is inert, unlike `pd_events.go`'s negative `activeTransfers` which fails the run. The skip precedes the stabilization gate, so a zero-delta tick counts as signal *loss* and resets the model's timer. See `docs/contributing/standards/invariants.md`.
...
- **INV-A GPU conservation**: For every node, `allocated_gpus + free_gpus == total_gpus`. Runtime verifier `PlacementManager.VerifyConservation()` (`sim/cluster/infra_placement.go`) walks nodes in sorted ID order and returns a descriptive error on the first violation. The node-level analogue of INV-4. See `docs/contributing/standards/invariants.md`.
...
- **NS-6 Catalog is authoritative and read-only** (#1733, a code-boundary rule at the CLI): a model runs **iff it is in the catalog**, and no run changes the catalog. (1) `cmd.resolveModelConfig` READS `<catalog>/models/<short-name>/config.json` — with a transition fallback to the flat `<catalog>/<short-name>/config.json` (#1774; `--catalog`/`BLIS_CATALOG` name the catalog CLONE ROOT) — inside the catalog located by `--catalog` / `BLIS_CATALOG` (#1731 — no default, no search path; a run naming no catalog is refused naming both forms) and refuses an entry absent from BOTH layouts, or a non-HF entry, **naming the path it belongs at** — no run-time HuggingFace fetch, no `run`/`replay`/`observe` creates or modifies a catalog file. (2) `--hardware`/`--tp` are REQUIRED on both `run` and `replay` (`cmd.requireDeploymentFlags`), refused **naming the missing flag** instead of inferred per-model from `defaults.yaml`. Binds the CLI only — Go-API callers constructing a config directly are unaffected. INV-6-preserving: supplying the flags `defaults.yaml` would have supplied reproduces the pre-#1733 stdout exactly. #1768 has since deleted that `defaults:` block outright (it had no consumer left), so a hand-maintained `defaults.yaml` still carrying one is refused at load (R10). See `docs/contributing/standards/invariants.md`.
...
**Catalog hygiene / NS-6 (#1733, R1 task S6 of tracker #1727)**: the two run-time behaviours that let the catalog drift and the deployment go unchosen are gone. **(1) No run-time HuggingFace fetch.** `cmd.resolveModelConfig` is now a single read of the catalog entry `<catalog>/<short-name>/config.json` (since #1731; it was a two-step read with an explicit per-model folder flag taking precedence) — with **no fallback**: an absent entry is a hard error naming the path it belongs at, and an entry that is not a HuggingFace config is refused *without being overwritten or deleted* (it may be an operator's hand-written entry with a fixable typo). The whole fetch family (`fetchHFConfig`, `fetchHFConfigFromURL`, the `fetchHFConfigFunc` test seam, `validHFRepoPattern`, the HTTP/redirect/size-limit machinery and the `os.MkdirAll`+`os.WriteFile` that wrote into `model_configs/`) is **deleted, not gated** — `TestNS6_NoRuntimeFetch_StaticGuard` fails if any of those symbols or a `huggingface.co` string literal reappears in `cmd/`'s production sources, or if `cmd/hfconfig.go` acquires a network import or a file-creating call. `GetHFRepo` survived #1733 as a caller-less provenance accessor for the `hf_repo` key and was **deleted by #1768** (below); `GetDefaultSpecs` is **deleted** — it existed only to infer the deployment. `blis observe` was already clean (it resolves no model config). **(2) `--hardware`/`--tp` are required.** The "early defaults resolution" block that looked them up per-model in `defaults.yaml` behind a `logrus.Warnf` and continued is replaced by `cmd.requireDeploymentFlags`, called **once, before any backend branch**, so it also subsumes the two per-backend `missing` checks (roofline / trained-physics) and cannot be skipped by a backend selection; the refusal names only the flag actually omitted. Because `run` and `replay` share `resolveLatencyConfig`, both refuse identically (INV-13). `DefaultConfig.GPU`/`TensorParallelism` stayed *declared* through #1733 (strict `KnownFields(true)` parsing rejects an undeclared key, R10, so the file could not be trimmed on its own) and were **removed together with the file's `defaults:` block by #1768** (below). The byte-identity golden (`specs/007-lora-control-plane/testdata/baseline_noop.json`, captured pre-#1733 from a run with **neither** flag) still matching is the INV-6 evidence: R1 changed which inputs are *required*, not any number. **Behavior changes on the record:** an uncatalogued model no longer runs (previously it fetched, ran, and left a new catalog entry behind); a run missing `--hardware` or `--tp` no longer runs (previously it warned and continued). The `zai-org/GLM-5.2` doc examples depended on the fetch and now name the committed catalog entry `zai-org/GLM-5.2-FP8`. All ~108 `blis run`/`blis replay` examples across `CLAUDE.md`, `README.md`, the mkdocs site, and `examples/` YAML headers pass both flags, enforced by `TestDocExamplesPassDeploymentFlags` (`docs/plans/` and `specs/` are excluded as historical records). Cross-repo prerequisites C3/C4 (every intended model catalogued) are human-coordinated in `blis-catalog`. S4 (#1731) has since landed: the refusal messages name `--catalog` / `BLIS_CATALOG` rather than the retired folder flag.

**Dead per-model deployment policy deleted / #1768 (R1 cleanup task of tracker #1767)**: `defaults.yaml`'s `defaults:` block — one `GPU` / `tensor_parallelism` / `hf_repo` triple per model — is **gone**, together with `Config.Defaults`, the whole `DefaultConfig` struct and the `GetHFRepo` accessor. It was dead on every run path from NS-6 (#1733): the deployment is a required operator input (`--hardware`/`--tp`, `requireDeploymentFlags`) and the model config is read from the catalog located by `--catalog`/`BLIS_CATALOG` (#1731). **A deletion of inert policy, not a data migration** — the `blis-catalog` `models/<name>/model.yaml` files carry model identity/provenance (so `hf_repo` has an equivalent home there) but carry **no** per-model GPU/TP defaults, and nothing consumed BLIS's, so nothing is lost. **Why the three land as one PR:** `KnownFields(true)` (R10) is **one-way** — an undeclared YAML key is a hard load error, an unsupplied Go field is legal and zero-valued — so trimming the YAML block alone is safe while deleting the struct field alone would break every load; and `GetHFRepo` read `cfg.Defaults[...].HFRepo`, a *compile* dependency. **Behavior change on the record (the only one):** a hand-maintained `defaults.yaml` that still carries a `defaults:` block now **fails to load** naming the field (`field defaults not found in type cmd.Config`) instead of parsing it and ignoring it — the intended outcome, since re-declaring the field to accept such a file would reinstate a config key with no consumer. The bundled file is trimmed and carries a comment pointing at the flags. **INV-6:** the deleted fields fed no number — verified byte-identical against `main` on a dense TP=1 run and an MoE `--dp 2 --enable-expert-parallel` run, and the pre-#1733 golden `specs/007-lora-control-plane/testdata/baseline_noop.json` still matches via `TestNoOpByteIdentity_AdapterBlindRunMatchesBaseline`. **INV-13:** `run` and `replay` share `loadDefaultsConfig`, so neither can drift. `TestNS6_ByteIdentityAnchor_GoldenModelDeployment` (#1733's cross-check that the golden run's explicit flags equalled what `defaults.yaml` would have inferred) is **removed** — it was the last reader of those keys and has no data left to read; a comment in `cmd/ns6_catalog_test.go` records where it went and that the INV-6 evidence it guarded is undiminished. Contracts live in `cmd/defaults_block_removal_test.go`: the trimmed file still parses with every surviving section intact, a stale `defaults:` block is refused (with a same-fixture-minus-the-block negative control), and a static guard rejects any reappearance of `GetHFRepo`/`DefaultConfig` in `cmd/`'s production sources or of a per-model `GPU`/`tensor_parallelism`/`hf_repo` key in `defaults.yaml`.
...
**Strict `hardware_config.json` parsing (#1728, R10)**: `parseHWConfig` was the last permissively-decoded config path in BLIS, so an unrecognized numeric key decoded to 0 — a plausible-but-wrong bandwidth/MFU/memory capacity with no diagnostic anywhere. `rejectUnknownHardwareCalibKeys` now rejects any key on a GPU entry that is not a `sim.HardwareCalib` field, naming **the offending key AND the GPU entry** (which `json.DisallowUnknownFields` cannot, and which would also reject the provenance keys). The accepted set is derived by **reflection over the struct's `json` tags** (R23: a new field is accepted with no parser change and cannot drift from what the decoder populates), plus the two documentation-only keys `_comment` / `_comment_interconnect`, which are accepted and ignored so the bundled file can keep calibration provenance next to the numbers it explains. Two offender classes are distinguished: an **unknown** key (the silent-zero case) and a **case mismatch** (`IntraNodeBwGbps`) — the latter is worth its own message because `encoding/json` matches field names case-insensitively, so such a key actually loaded CORRECTLY before this change; it is rejected anyway (two spellings of one field resolve last-wins, and the file is otherwise one letter from a real typo) with the canonical spelling named. The scan is **file-wide** (like the sibling legacy-key guard, and unlike `GetHWConfig`'s per-GPU `ValidateInterconnect`): a present-but-unusable value only matters for the entry in use, whereas a misspelled key is a file-integrity defect nothing else will surface. Offenders are sorted, so the diagnostic is byte-identical across runs (INV-6). The **value decode is unchanged**, so every valid config — including the bundled one — parses exactly as before (INV-6, verified byte-identical stdout across dense/TP/roofline/MoE-DP-EP runs); only invalid input changes from silent-zero to hard error. Deliberately out of scope: the permissive external-payload parsers (HF `config.json`, `otel_convert.go`, `weka_convert.go`), which tolerate unknown fields by contract.

**Inter-node network cost (#1530, trained-physics only)**: the two trained-physics communication bases divide byte volume by an **effective** link bandwidth rather than always by `bwHbmUs`. When a collective's participant group does not fit inside one node, the divisor becomes `bwHbmUs / spanScale` — a **re-scale in place** of the existing basis, never a separate additive term (an additive term would double-charge a `DP>1` MoE instance whose all-to-all `moeDispatchBasis` already prices). `tpAllReduceBasis` covers all three TP-group collectives (`tTpAttention`, `tTpDenseFFN`, and the `dp==1` `tMoEReduce`); `moeDispatchBasis` covers the expert dispatch/combine. Two penalty shapes, both of the form `1 + (r-1)·crossHops/(G-1)` with `r = IntraNodeBwGBps/InterNodeBwGBps`: a **ring** penalty `1+(r-1)(n-1)/(G-1)` derived from NCCL's hierarchical (two-level) multi-node all-reduce — applied to the TP group *and* to the `allgather_reducescatter` MoE family, whose volume basis is ring-shaped (2 phases × `(G-1)/G`) — and an **all-to-all** penalty `1+(r-1)(G-g)/(G-1)` for a true all-to-all backend (`deepep_*`/pplx/mori/flashinfer), where a rank's data must reach every peer with no reduction on the way, so far more of it leaves the node. Both are exactly `1.0` at `n==1` or `r<=1`, and monotone in `r` (AC-2). **The topology is placement-derived, never a flag**: `PlacementManager.PlacedGPUsPerNode(gpuIDs)` reports the size of the node(s) an instance actually occupies, and `ClusterSimulator.applyPlacementTopology` stamps it onto the per-instance `SimConfig` at all three placement sites (startup / deferred `NodeReadyEvent` / autoscaler scale-up, R23), immediately after the per-instance GPU type (#893), KV capacity (#1522) and cost (#1529). A guard test asserts no CLI flag declares it — a declared node size can contradict real `node_pools` placement. It lives on `ModelHardwareConfig.NetworkTopology` (a latency input like TP/DP/`MoECommBackend` — R16, not a SimConfig-level sub-config), supplied via `sim.WithNetworkTopology` — a new `ModelHardwareOption` variadic, mirroring `WithKVOffload` on `NewKVCacheConfig`, so all 211 `NewModelHardwareConfig` call sites are untouched (R4). Production writes the field directly at the placement sites (placement is not known until after the config exists); the option serves tests/standalone construction and is what the constructor's `validate()` guard covers. **Exported surface is deliberately minimal**: everything crossing a package boundary is exported (`NetworkTopology` + `NewNetworkTopology`/`NodesSpanned`/`MembersPerNode`/`IsKnown` for `sim/latency`+`sim/cluster`; the four `HardwareCalib` interconnect methods; `ModelHardwareOption`+`WithNetworkTopology`; `ClusterSimulator.MaxNodesSpanned` for `cmd`; and `LatencyBackendRoofline`/`LatencyBackendTrainedPhysics`, which *remove* three duplicated string literals). What does not cross one is package-private: `NetworkTopology.validate` and `PlacementManager.placedGPUsPerNode` are unexported. **Fabric speeds are hardware calibration**: `HardwareCalib.IntraNodeBwGBps`/`InterNodeBwGBps` (per-GPU *effective unidirectional* GB/s, committed for H100 450/50, A100 300/25, L40S 32/12.5 in `hardware_config.json` with provenance comments) — only their **ratio** is used, so the absolute scale cancels; both must be set or neither. Validation lives on `HardwareCalib.ValidateInterconnect()` and runs at BOTH the `GetHWConfig` load boundary (so a malformed file fails identically under either backend, R23) and the trained-physics constructor (so a programmatically-supplied calib is covered too): a half-set bandwidth pair or an unusable value is a hard error, never a silent clamp (R1). A committed-file companion invariant (`sim/latency/interconnect_calib_file_test.go`) enumerates the file's own keys and asserts every entry loads, is complete, and has a plausible ratio. **Second, size-independent half — analytic hop-count × α_hop × S (`InterNodeHopLatencyUs`, #1694, restructuring the flat per-collective term #1667 shipped)**: the fixed launch + fabric round-trip of a cross-node collective, charged as `n_steps · α_hop · S` per comm unit and skipped when a step communicates no tokens. **`n_steps` is the analytic PER-COLLECTIVE hop count**, derived purely from the collective algorithm and `NetworkTopology.NodesSpanned()` with **zero free parameters** — replacing the flat #1667 constant, which was node-span-INVARIANT and under-charged a wide span by ~`(nodes−1)×`. Two counts: `crossNodeRingHops` = `2·(nodes−1)` for ONE WHOLE all-reduce (reduce-scatter + all-gather), consumed by the TP leg (one comm unit IS one all-reduce); `crossNodeAll2AllHops` = `(nodes−1)` for ONE SINGLE-PHASE collective, consumed by the MoE dispatch/combine leg for BOTH comm families. The per-layer collective count is a separate factor (one comm unit per TP collective; `moeDispatchCollectivesPerLayer`=2 for MoE dispatch+combine), so the MoE latency term is `2·(nodes−1)` per layer — **family-independent**: the family split is real for the bandwidth VOLUME (`spanScalesFor`) but SPURIOUS for the hop count, since the all-gather family's dispatch (all-gather) and combine (reduce-scatter) are each one single-phase collective. Charging the all-gather family a whole ring here would double-count (`4·(nodes−1)`/layer) — the ring's two phases ARE the two collectives `moeDispatchCollectivesPerLayer` counts. **`α_hop` (`HardwareCalib.InterNodeHopLatencyUs`) is a per-fabric constant**, **0 (uncalibrated, not charged) in the bundled config**, deliberately: BLIS has no measured per-hop value and a guessed constant would sit in front of every multi-node estimate. It must come from an independent NCCL microbenchmark reused across fabrics — never back-solved from one run's residual (anti-overfitting guardrail; `TestCommittedHardwareConfig_LatencyIsUncalibrated` pins the 0). **Migration guard (R1):** the pre-#1694 per-collective `InterNodeLatencyUs` key is REJECTED at the `parseHWConfig` load boundary (`rejectLegacyInterNodeLatencyKey`) naming the offending GPU + the new key. Since #1728 the whole file parses strictly, so the legacy key is *also* caught as an unrecognized key — but the specific guard runs FIRST and keeps precedence, because a bare "unknown key" would lose the migration hint: the unit changed (per-collective → per-hop), so it is a recalibration, not a rename — divide the old value by the hop count. (Before #1728 the permissive decoder would have dropped the key to 0 silently, with the bandwidths alone satisfying `HasInterconnectCalibration` so `warnIfCrossNodeUnpriced` stayed quiet.) **CLI diagnostic (R1):** `resolveCommSerializationFactor` warns (not fatal) when `S>1` but the term provably cannot fire — non-trained-physics backend, or no `--policy-config` node pools — so a requested penalty never silently vanishes. **`S` (`ModelHardwareConfig.CommSerializationFactor`, `--comm-serialization-factor`, default 1.0) is a deployment-regime multiplier** — eager/no-overlap (`S≫1`) vs CUDA-graphs/overlap (`S≈1`) — kept strictly SEPARATE from `α_hop` (folding it in would make a graphs-on deployment inherit a graphs-off fabric constant). It multiplies ONLY this latency term, is supplied via `WithCommSerializationFactor`, re-supplied on both `run` and `replay` (not header-round-tripped, INV-13), and `--enforce-eager` requires an explicit `S>1` (BLIS ships no fitted eager magnitude). `EffectiveCommSerializationFactor` clamps `S≤1`→1.0 (monotone; can only raise cost). So the latency clause is exercised only synthetically today — no shipped config makes the term fire. Like the bandwidth half it rides β₄/β_EP, so `α_hop` is calibrated in that frame (`β·units·n_steps·α_hop·S`), and it stands alone (a latency-dominated fabric is meaningful, so it is not paired with the bandwidths). **Inert at three independent gates** (INV-6/INV-BC-DP1): no node pools ⇒ topology unknown; group fits a node ⇒ `n=1`; no fabric calibration ⇒ `r=1`. In each case the divisor is `bwHbmUs` itself, bit-for-bit. **`blis run` only, fenced on BOTH paths** (INV-13). #1530 offered two parity designs — round-trip the cross-node signal so replay reproduces the run's step times (a), or keep a loud rejection (b) — and this is **(b), extended to cover the trace path**. The *config* path: `blis replay` `logrus.Fatalf`s on `node_pools` and `observe` builds no simulator, so a non-inert topology is unreachable there. The *trace* path is NOT covered by that guard, and — verified empirically against `main`, where the same export-and-replay round-trip returns identical metrics (0 of 27) while with this feature 16 of 27 differ — this feature is what first makes that divergence observable in step time, so it is fenced here rather than inherited. `blis run --trace-output` records the widest instance node span in the TraceV2 header (`MaxNodesSpanned`, `omitempty`; the writer normalizes a span of 1 to 0 via `cmd.crossNodeSpanForTrace`, since `omitempty` drops only 0 — so a run without multi-node placement writes a byte-identical header) and `blis replay` hard-errors on any trace reporting more than one node. **The header field is a REFUSAL signal, not a reconstruction input**: replay reads it only to fail loudly, never to rebuild a fleet (reproducing cross-node timing would need per-instance topology, not one fleet-wide maximum) — so this remains design (b), and the asymmetry is CLOSED rather than merely documented. Both fences have dedicated subprocess tests plus negative controls (field absent, and the boundary value 1), and the header field has its own round-trip / omission / byte-identity tests. **Behavior change on the record:** replaying a newly-exported spanning trace is now a hard error where it previously produced (wrong, single-node-speed) numbers. Backward compatibility is preserved for every trace written before this feature — no key ⇒ 0 ⇒ accepted — verified by replaying a `main`-exported spanning trace under the new binary with 0 of 27 metrics differing. That is also the guard's known limitation: it protects traces exported from this version onward, since a pre-existing trace's span cannot be detected retroactively. **Diagnostics (R1)**: `warnIfCrossNodeUnpriced` latches one warning PER CAUSE (so a mixed fleet reports each distinct reason once) for the three ways a genuinely spanning placement ends up unpriced — a backend with no comm term, an unresolvable node size, or an uncalibrated interconnect — plus a fourth for an implausible (>1000×) ratio, a deliberate unit-error detector rather than a plausibility judgement (real hardware spans ~2× on PCIe nodes to ~75×). The diagnostics score the REAL distinct-node count, not `NodesSpanned`, so they still fire when the node size itself could not be resolved — otherwise they would go quiet exactly when most needed. **Calibration honesty**: magnitude is modest — the TP comm term rises ~1.53× for TP=16 on 2×8 H100 at `r=9`, ≈+10% on total step time for a mid-size dense model — because hierarchical all-reduce moves the same bytes and only the reduced `S/g` chunk crosses the fabric; a **flat** multi-node ring would instead be throttled to ≈`r` (9×), and measured two-node H100 bus-bandwidth degradation (~1.3–1.5×) is why the hierarchical model is the one used. `IntraNodeBwGBps` should be read as "the on-node link speed β₄ was calibrated against", since β₄ already absorbs the NVLink/HBM ratio. Known approximations, each with a follow-up: per-hop launch/RTT cost is now MODELED as `n_steps·α_hop·S` (#1694) but ships uncalibrated (`α_hop=0`), and is plausibly the *dominant* cross-node effect at decode message sizes — supplying a measured `α_hop` from an NCCL microbenchmark is the calibration bar (**#1694**); the fabric is keyed by `gpu_type` rather than by pool, correct only while #1529's unique-`gpu_type` rule holds (**#1662**); the roofline backend prices no communication at all (**#1663**); the all-to-all penalty sums rather than overlaps the on/off-node portions and ignores DeepEP's per-node RDMA coalescing (both pessimistic); and the MoE group's span is extrapolated from the placed node size, since BLIS places a TP group. **#1548 made the expert dispatch/combine TERM reachable** (`--enable-expert-parallel` now charges it, at the on-node rate). Its **cross-node pricing** across the EP group remains inert even now that `node_pools`+`--dp>1` is supported (#1553): DP-as-placement forms the EP group across `N` *independently placed* single-node replicas, and #1530's placement-derived cross-node cost is scoped to the collectives WITHIN one instance's TP group — it does not cross the inter-replica boundary. `blis run` warns rather than presenting the on-node price as complete — exactly the per-leg split #1530 prescribes.
...
**Catalog location / S4 (#1731, R1 task S4 of tracker #1727)**: there is now **exactly one way** to tell BLIS where a model config lives, and it does not depend on the working directory. Before this, two mechanisms overlapped: the explicit `--model-config-folder` flag, and an implicit `model_configs/<short-name>/` lookup rooted at `filepath.Dir(--defaults-filepath)` — which, with that flag's relative `defaults.yaml` default, meant `blis run` silently worked only from the repository root. A relative default inherits that hazard and contradicts NS-6 (nothing about the model is inferred). **The catalog is now located by `--catalog <path>` or the `BLIS_CATALOG` environment variable and nothing else — no default, no search path, no remote fetch of the catalog itself.** `--catalog` wins when both are set (an explicit CLI input beats the environment) and the override is announced on stderr, so the choice is history-visible; a run with **neither** is refused **naming both forms**. `--model-config-folder` is **retired** — pointing `--catalog` at a scratch clone covers the same "use my own config" need. **Shape change worth knowing:** `--catalog` names the catalog **ROOT** (one directory per model), where the retired flag named a directory holding `config.json` *directly* — so `--model-config-folder model_configs/deepseek-v2-lite` becomes `--catalog model_configs`. The bundled `model_configs/` tree is a valid catalog. **Code:** `cmd.catalogRootFrom(flagValue, envValue)` is the pure precedence law (both inputs are arguments, so it is table-testable without touching globals or the environment); `cmd.resolveCatalogRoot()` is the production wrapper that reads the `catalogPath` flag var + `os.Getenv("BLIS_CATALOG")` — mirroring the single `HF_TOKEN` read that was `cmd/`'s only environment access — and additionally refuses an unreadable or non-directory root naming both forms (R1: a mistyped catalog path must not be deferred into a per-model "not in the catalog" message that blames the model). `resolveModelConfig(model)` composes those with the injectable core `resolveModelConfigInCatalog(model, catalog)`, and `catalogModelDir` replaces `bundledModelConfigDir` — an **empty** catalog root is an error there rather than a relative path, so the retired working-directory default cannot creep back in. The single overloaded `modelConfigFolder` package var is split into `catalogPath` (the `--catalog` input) and `modelConfigDir` (the resolved entry directory, a `resolveLatencyConfig` side effect). **INV-13:** the flag rides the shared `registerSimConfigFlags`, so `run` and `replay` take it identically and resolve through the same helper; `observe` takes neither form (it resolves no model config). **INV-6:** only the *name* of the path changed — `TestNoOpByteIdentity_AdapterBlindRunMatchesBaseline` still matches `specs/007-lora-control-plane/testdata/baseline_noop.json`, a golden captured before this change, and the two spellings (`--catalog` vs `BLIS_CATALOG`) give byte-identical stdout. **Enforcement:** `TestS4_ModelConfigFolderRetired_StaticGuard` makes AC-3's grep executable over `cmd/`'s production sources, `TestS4_ModelConfigFolderFlag_NotRegisteredOnAnyCommand` walks the whole cobra tree, and `TestS4_CatalogEnvVarReadOnlyInResolver` pins that exactly one production file names the environment variable. **Documentation choice on the record:** the ~108 `blis run`/`blis replay` examples across `CLAUDE.md`, `README.md`, the mkdocs site and `examples/` were **not** each given a `--catalog` flag (unlike #1733's `--hardware`/`--tp`, which have no environment-variable form). `BLIS_CATALOG` exists precisely so the location is stated once, so the entry points (`docs/index.md`, `docs/getting-started/quickstart.md`, `installation.md`, `tutorial.md`, and the header of this file) instruct a one-time `export BLIS_CATALOG=$PWD/model_configs` and say the examples assume it. `TestDocExamplesDocumentTheCatalogEnvVar` pins that instruction so a future edit cannot silently delete the thing that makes those examples runnable. Cross-repo prerequisite C1 (the `blis-catalog` repository holding `catalog/models/`) is human-coordinated.

**Catalog location convention settled / #1774 (R1 cleanup of tracker #1767)**: `--catalog` / `BLIS_CATALOG` name the catalog **CLONE ROOT**, and model resolution appends `models/`. S4 (#1731) left BLIS and the authoritative `blis-catalog` repository disagreeing about what the flag pointed at: BLIS implemented **entries-root** semantics (`<catalog>/<short-name>/config.json`, no `models/` level) while `blis-catalog` stores configs at `<clone-root>/models/<name>/config.json` with `workloads/`, `devices/`, `hardware/` and `networks/` as **sibling** namespaces under the same root — so an operator had to pass `<clone-root>/models` while #1767 and the catalog README described the clone root. The clone root wins because those siblings only compose off a single root, and the upcoming sibling readers (#1769 workloads, #1770 devices) plus R2 (coefficients) all inherit this one choice — which is why it is settled before any of them. **Transition fallback (INV-6):** `cmd.catalogModelDirs` (renamed from the singular `catalogModelDir`) is a **pure ordered candidate list** — `<catalog>/models/<short-name>` first, then the flat `<catalog>/<short-name>` — and `cmd.readCatalogEntry` is the one place that reads, picking the first candidate that HAS a `config.json`. Without the flat candidate this change would instantly break `export BLIS_CATALOG=$PWD/model_configs` (that bundled tree has no `models/` level) and every `cmd/*_test.go` flat fixture; it also keeps the pre-#1774 invocation `--catalog <clone-root>/models` working, since `<clone-root>/models/models/<name>` is absent and resolution falls back. **#1771 deletes the bundled tree and this fallback together.** **Only ABSENCE falls through (R1, NS-6):** a candidate whose `config.json` exists but is unreadable or malformed IS the entry and is reported naming that path — the fallback must never substitute a different model's config for a broken one, which would turn a typo in a catalogued entry into a successful run of the wrong model. A model absent from both layouts is refused naming **every path looked at** plus the canonical `models/` path to add. **Path semantics, now stated in the flag help and `docs/reference/configuration.md`:** a **relative** value is resolved against the process working directory, an **absolute** value is used as given, and neither is rewritten — so #1732's provenance `path` (recorded **verbatim**) is not categorically CWD-relative, and an absolute `--catalog` is the attributable choice (`docs/guide/results.md`). **INV-6/INV-13:** the layout is not an input to the simulation — `TestRunCmd_CatalogCloneRootLayout_ByteIdenticalToFlat` and `TestReplayCmd_CatalogCloneRootLayout_ByteIdenticalToFlat` (`cmd/catalog_layout_test.go`) drive the SAME `config.json` bytes through both layouts on both commands and assert byte-identical stdout; the resolver is shared by `run`/`replay` (`observe` resolves no model config). **Cross-repo (human-coordinated):** `blis-catalog/README.md` still says simulator catalog loading is unimplemented and instructs `BLIS_CATALOG=<clone-root>` — the instruction is now correct and the "unimplemented" note needs retiring.

**Catalog provenance in the results file / S5 (#1732, R1 task S5 of tracker #1727)**: a results file used to say nothing about **which catalog produced it**. Now that the catalog is an explicitly located, versioned directory (#1731 + `blis-catalog`), a result must be attributable and an *experiment* (uncommitted edits to a `config.json`) must be distinguishable from a *reproducible run*. `MetricsOutput` gains a `catalog` block — `sim.CatalogProvenance{Path, Revision, Dirty}` — recording the resolved catalog root, its git revision, and whether the catalog subtree was dirty. **File-only, exactly like #1583's `cache_hit_rate`:** it is supplied through a new variadic `sim.EmitOption` seam (`EmitOutput(output, path, opts...)` + `sim.WithCatalogProvenance`) and assigned inside `EmitOutput`'s file branch, AFTER the stdout marshal — provenance legitimately varies with catalog git state, so it must never enter the byte-identical stdout channel INV-6 protects (the option is a no-op with an empty output path, and variadic so all pre-existing call sites are untouched, R4). **Capture is net-new and lives in `cmd/`** (`cmd/catalog_provenance.go`), not `sim/`: `sim/` is a library that must not shell out, and the catalog path is a CLI input. It shells out to `git -C <catalog> rev-parse HEAD` plus `git -C <catalog> status --porcelain -- .`, each under a 10 s context timeout with `GIT_TERMINAL_PROMPT=0`. **The dirty check is deliberately scoped to the catalog SUBTREE** (pathspec `.`), because a catalog is commonly a subdirectory of a larger checkout (the bundled `model_configs/` is one) where an unrelated edit elsewhere would mark nearly every run an experiment and make the flag useless; untracked files count as dirty (an uncommitted new entry makes the catalog differ from its committed revision, which is precisely the reproducibility question). **Degrades, never fails (R1):** an absent, non-git, commit-less or unreadable catalog records the path with `Revision = sim.UnknownCatalogRevision` (`"unknown"`, a recorded sentinel so "asked, unknown" is distinguishable from "never populated") and `Dirty = false`, plus one stderr `logrus.Warnf` — a revision is never invented and no run is failed over metadata. An unresolved catalog (empty path) yields **no block at all** rather than one claiming an empty path. **INV-13:** `cmd.catalogProvenanceEmitOptions(outputFilePath, catalogRoot)` is the single shared helper both `blis run` and `blis replay` pass at their `EmitOutput` sites (R23), so neither can drift into recording provenance the other omits; `blis observe` resolves no model config and is unaffected. The catalog root reaches the emit site via the new `resolvedCatalogRoot` package var, a `resolveModelConfig` side effect in the same spirit as `modelConfigDir` (re-resolving at the emit site would re-emit the `--catalog`/`BLIS_CATALOG` precedence announcement). **INV-6:** without `--metrics-path` no git subprocess runs at all; with it, stdout is byte-identical between a clean-catalog and a dirty-catalog run (pinned end-to-end for both commands by `TestCatalogProvenance_RunCLI_RecordsCatalogAndKeepsStdoutIdentical` / `TestCatalogProvenance_ReplayCLI_RecordsCatalog`, whose fixture edit is whitespace-only so the model config — and therefore every number — is unchanged). **Scope boundary on the record:** the block is added to `MetricsOutput` (the `--metrics-path` file), which is what the issue declares; `--results-path`'s `[]workload.SimResult` is a per-request array with no header object and is left alone.

**Layer count from a block-type array (#1729, NS-4)**: `HFConfig.ResolveNumLayers` is the single source of truth for the total layer count — the `num_hidden_layers` scalar when present and non-zero, else `HFConfig.BlockTypeLayerCount()` = `len(layers_block_type)` (`latency.LayersBlockTypeField`). Some configs (Nemotron-3-Ultra-550B) declare no scalar and express the count only as the per-layer block-type list; BLIS read the scalar, got 0, and aborted before any simulation (`NumLayers must be > 0`). The scalar always wins when it answers, so every catalogued model is byte-identical (INV-6, verified across dense/MoE/MLA/quantized/EP shapes); a present-but-negative scalar is NOT overridden (bad input keeps failing loudly at both backends' validators); neither source ⇒ 0 ⇒ a loud failure, never a silent 0. **Where that failure is raised changed in #1777**: `GetModelConfigFromHF` now refuses at the parse boundary with a diagnostic naming BOTH keys the resolver consults and what is wrong with each (absent / `0` / empty list / wrong JSON type), instead of deferring to the per-backend `ModelConfig.NumLayers must be > 0` — that message names a Go struct field rather than either config key, and a present-but-wrong-typed `layers_block_type` (a bare string where a list belongs) is indistinguishable from an absent one by the time a validator runs, so an operator could be shown "layer count missing" while looking at a config that visibly states it. Scope is the no-evidence case only: a NEGATIVE scalar *answered* (bad input, not missing input) and still passes through, so the validators report the actual value. Both backend validators keep their own zero-layer check as defense in depth for a `ModelConfig` built by any other route. **`HFConfig.Raw`'s value shape is now a stated precondition** (#1777): exactly what `json.Unmarshal` into `any` produces — numbers as `float64` — because `Raw` is exported and `GetModelConfigFromHF` accepts a caller-built `*HFConfig`, so a caller decoding with `json.Decoder.UseNumber()` supplies `json.Number` and *every* reader (`GetInt`/`GetBool`/`GetString`, the `getInt` closure, `parseQuantizationConfig`) asserts a concrete type. Teaching `GetInt` alone to accept `json.Number` would **half-decode** a config (a layer count that resolves beside a `hidden_size` silently read as 0), which is worse than refusing — so the contract is documented and `layerCountEvidence` gives that case its own clause naming the *decoder*, instead of reporting a genuine JSON number as `not a number` (a self-contradiction that sends the caller editing a config value that is already correct). Invalid-input diagnostics only — every valid config parses byte-identically (INV-6). `cmd.isHFConfig` accepts the same evidence (a non-empty list) so a config the parser can read is not rejected one layer up and re-fetched. **Scope: counting only** — every entry is charged as one full transformer layer whatever type it names, so a hybrid attention/Mamba list is priced as all-attention (PESSIMISTIC; `logrus.Warnf` on stderr whenever the count is derived, R1 — the warning gate reads the resolver's own `numLayersScalar` predicate ("present AND non-zero"), so a config carrying `"num_hidden_layers": 0` beside a valid list warns like any other derivation instead of being reinterpreted silently). Per-type layer groups / tallies are a later release (R4a). Parse-time only, so `run`/`replay` share it (INV-13, no trace-header field) and `observe` is unaffected.
...
**Offload cache hits reduce billed prefill (`sim/kv.OffloadCache` + `sim.ReloadReportingKVStore`, #1699, whole-prefix credit #1706)**: the fix for a billing bug where a CPU/secondary offload hit moved the reported `cache_hit_rate` but left **every** latency metric byte-identical (changing `cpu_bytes_to_use` swung hit-rate 0.077→0.367 with TTFT frozen). Root cause: `FormBatch` Phase 2 computes a new request's billed prefill work (`numNewTokens`, the sole `StepTime` input) from the **GPU-only** `GetCachedBlocks` **before** the CPU→GPU reload runs *inside* `AllocateKVBlocks` — so a reload that lands prefix blocks on the GPU (and bumps `CacheHits`) never reduced `NumNewTokens`, and a genuine hit was billed as a full recompute. This is BLIS's missing analogue of vLLM's `num_new_tokens = num_tokens − (num_local_computed + num_external_computed)` (`scheduler.py`): the external/offload hit must shrink the tokens sent through the prefill forward pass. **Mechanism (#1706 reorder to vLLM's ordering):** an optional `sim.ReloadReportingKVStore` interface exposing the **PURE pre-allocation query** `ReloadablePrefixEnd(req, startIndex) (reloadableEnd, ok)` — the analogue of vLLM's `get_num_new_matched_tokens` on the waiting-queue path — implemented by **both offload stores** (`OffloadCache`, the #1590 chain; and the legacy `TieredKVCache`, `--kv-cpu-blocks`, the structural twin with the identical bug) but NOT by the non-offload single-tier `KVCacheState` (same inert-for-others type-assert pattern as the `DeferrableKVStore` step-boundary deferral above, #1591). The query reports the token boundary to which a same-step CPU→GPU reload WOULD extend the request's GPU-cached prefix, WITHOUT committing or mutating any tier — it walks the uncached tail like `consultAndReload`/`reloadPrefixFromCPU` but only COUNTS the contiguous run of GPU-resident-or-CPU-resident (same-step reloadable) blocks, stopping at the first `cpuHitPending`/`cpuMiss`/end. A **secondary-tier-only** run is NOT counted (it stays on the H3 step-boundary deferral path, `DeferrableKVStore`, which re-examines it over steps until it becomes CPU-resident). Gated to genuinely-new prefill admissions (`!running && !IsDecodeSubRequest`, the class `FormBatch` credits — a running continuation bills incrementally against `ProgressIndex`, a PD decode sub-request is admitted via the decode branch); returns `(startIndex, false)` otherwise. **`FormBatch` Phase 2 folds the reloadable boundary into the computed baseline BEFORE the chunk/budget clamp** (mirroring vLLM `num_computed_tokens = local + external`, sized before the `long_prefill_token_threshold` cap): `computedStart = max(startIndex, reloadableEnd)`, `numNewTokens = InputLen − computedStart` then clamped by `PrefillTokenThreshold`/token-budget/`MaxModelLen`, `endIndex = computedStart + numNewTokens`; `AllocateKVBlocks(req, startIndex, endIndex, cachedBlocks)` runs the reload internally, which — because `endIndex ≥ computedStart == reloadableEnd` — always commits the full reloaded prefix `[startIndex, computedStart)` and allocates only the uncached tail `[computedStart, endIndex)`. So `NumNewTokens = numNewTokens` (the uncached remainder, billed and chunked) and `ComputedTokens = endIndex` (progress past the WHOLE reloaded prefix). Because the fold-in precedes the chunk cap, the whole reloaded prefix is credited **even under chunked prefill** — the query is pure (no one-shot consumption; the actual reload + commit still happens inside `AllocateKVBlocks`), so nothing to leak on a failed alloc or a preempt→re-prefill, and the old `billedTokens = endIndex − min(newStart, endIndex)` one-chunk special-case is gone. **Covers BOTH offload paths** (both funnel through `allocateThroughChain`/`reloadPrefixFromCPU`): the CPU-only synchronous reload and the resolved-secondary-deferral (H3) admit (whose prefix is CPU-resident by the time it is admitted). **Scope:** prefill-shrink only — the CPU-only path stays a same-step optimistic reload (no explicit transfer latency), the secondary path keeps its H3 one-step load cost; routing CPU-tier hits through a one-step load (vLLM `WAITING_FOR_REMOTE_KVS`) is a documented follow-up. **#1706 resolves the former one-chunk cap** — vLLM's pre-cap ordering now credits the whole reloaded prefix, so a long-context offload workload (e.g. weka Kimi ~110K ISL at the 2048-token default chunk) realizes the full credit rather than the single-chunk share (≈ `chunk/prompt`). A documented edge: if the pure query and the mutating reload diverge under GPU/CPU free-block exhaustion (`maxReloads`/`popFreeBlock`), the reload is partial and batch formation may slightly over-credit — matching vLLM's model (credit external, then allocation failure preempts). **INV-6:** the non-offload single-tier `KVCacheState` doesn't implement the interface ⇒ `reloadReporter` is nil ⇒ `computedStart == startIndex` ⇒ the fold-in is dead code ⇒ a run without offload is byte-identical. (Both offload stores DO implement it — turning it on is the intended, output-changing case.) **INV-13:** the fold-in is a pure function of deterministic allocation state (no RNG/wall-clock) ⇒ replay reproduces it. `ReloadsPerformed()` exposes the existing `reloadCount` (#1586 diagnostic family) — the count of blocks billed as a hit rather than a recompute. Files: `sim/kv_store.go`, `sim/kv/offload_chain.go`, `sim/kv/offload_deferral.go`, `sim/kv/tiered.go` (legacy path), `sim/batch_formation.go` (+ `sim/kv/offload_chain_test.go`, `sim/kv/offload_deferral_test.go`, `sim/kv/tiered_test.go`, `sim/batch_formation_reload_test.go`; e2e `sim/cluster/offload_deferral_e2e_test.go`).
