...
- **Conservation formula**: the canonical INV-1 form is the 12-term CLUSTER equation; the 5-term form `injected == completed + still_queued + still_running + dropped_unservable + timed_out` is the SINGLE-INSTANCE specialisation only. For a cluster run add the seven cluster-only buckets (`routing_rejections`, `gateway_queue_depth`, `gateway_queue_shed`, `gateway_queue_rejected`, `gateway_evicted`, `gateway_expired`, `encode_routing_rejections`) — asserting 5 terms against cluster output is incomplete and will not catch a request lost to a gateway or routing bucket. See canonical INV-1 in `docs/contributing/standards/invariants.md`. `parse_blis_output` does NOT extract `dropped_unservable` or `timed_out` — parse them separately.
...
- Verify INV-1 conservation for every run: 5 terms (`completed + still_queued + still_running + dropped_unservable + timed_out`) for a single-instance run; all 12 terms for a cluster run (add `routing_rejections` and the six gateway/encode buckets)
