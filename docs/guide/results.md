...
| **Dropped at Decode KV** | count | Decode sub-requests dropped because the decode instance had insufficient KV capacity. Only printed when > 0. In the `PDMetrics` struct this field is `DroppedAtDecodeKV`; in the Anomaly Counters stdout section it appears as `Dropped KV Allocations` (same counter). |
