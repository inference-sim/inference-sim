...
- **Routed-expert weight/compute** are scoped to the flattened MoE group `moeGroup = TP·DP` via the `ExpertPlacement` seam: each GPU holds `numExperts/moeGroup` full-expert-equivalents. This replaces a batch-dependent heuristic, so MoE step time at `DP=1` intentionally differs from pre-DP/EP BLIS (a deliberate fidelity fix). Dense models at `DP=1` are byte-identical (INV-BC-DP1). **Step time uses `TP·DP` in both EP modes**, which is the EP-**on** sharding: with EP off vLLM replicates every expert per DP rank, so the faithful per-GPU figure is `numExperts/TP`. The capacity model makes exactly that distinction since #1656 (below), so at `DP>1` with EP *off* the two models disagree by `DP` on routed-expert bytes. Unreachable today — `blis run` gives every DP replica `DP=1` (#1531, making `moeGroup = TP`) and `blis replay` rejects MoE `--dp>1` (#1556) — and tracked for #1548, which owns the EP-mode step-time toggle.
