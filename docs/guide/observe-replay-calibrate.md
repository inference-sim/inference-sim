...
- **Validity boundary.** The reconstructed sim makespan (`send + simE2E`) is a physical sim timeline only for **fixed-mode replay** (the standard observe→replay→calibrate path, arrivals pinned from the trace). Under **closed-loop / concurrent-session replay** the sim regenerates the arrival schedule (round N+1 depends on the sim's completion of round N), so the real send schedule is not the sim's arrival schedule — treat the number as an open-loop metric there.
    - **`calibrate` cannot auto-detect this in every case.** It warns when the trace header carries `session_context_growth` (a delta/accumulate corpus), but a **plain trace replayed with `--session-mode closed-loop`** produces no such header marker, and the replay mode is a `blis replay` CLI choice not recorded in the trace or `SimResult`. In that case the throughput block is emitted with **no warning** and is silently open-loop-invalid. The reliable contract is on the operator: **only calibrate throughput against a fixed-mode replay.**
