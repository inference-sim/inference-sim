...
| **vLLM scheduler internals** (batch formation algorithm, preemption policy) | **Simplified** — observed through batch composition, not modeled directly | Step time depends on *what* is in the batch, not *how* the batch was formed | Lost: Preemption overhead (swap-in/out latency), scheduler iteration overhead that scales with queue depth. These are typically <5% of step time but may matter at very high preemption rates. |
...
| vLLM scheduler internals | Yes — we observe outputs | Marginally — preemption overhead | No | Low (<5% typically) | High (scheduler state model) | Partially |
...
| Method (behavioral description) | Research Scope | Integration Strategy |
|--------|---------------|---------------------|
| **Step-time estimation**: given a batch of requests, return the predicted step duration in microseconds | **Primary target** — replaces the 2-feature, 3-coefficient linear regression | Data-driven prediction (blackbox replacement only; roofline is unaffected) |
| **Queueing-time estimation**: given a request, estimate time spent waiting in queue | **In scope** — currently uses 2 alpha coefficients | Data-driven or improved coefficient-based prediction |
| **Output token processing time**: constant overhead per output token | **In scope** — currently uses 1 alpha coefficient | Data-driven or improved coefficient-based prediction |
| **Scheduling processing time**: constant overhead per scheduling decision | **In scope** — currently returns 0 (not modeled) | Data-driven prediction or calibrated constant; the current 0-return may underestimate scheduling overhead at high queue depths |
| **Preemption processing time**: constant overhead per preemption | **In scope** — currently returns 0 (not modeled) | Data-driven prediction or calibrated constant; the current 0-return ignores swap-in/out latency |
...
- **Queueing-time estimation** affects TTFT directly (arrival-to-first-token delay). Currently uses 2 alpha coefficients. Research ideas may propose data-driven improvements or retain the current implementation if the E2E mean error is dominated by step-time errors.
- **Output token processing time** affects ITL (added to each decode step). Currently uses 1 alpha coefficient. May benefit from data-driven calibration.
- **Scheduling and preemption processing time** currently return 0 (not modeled). Research ideas may propose calibrated constants or data-driven predictions based on queue depth and preemption frequency observed in the ground-truth data.

**Minimum viable scope:** A research idea that improves only step-time estimation while retaining the current implementations for the other 4 methods is acceptable — the success criterion is workload-level E2E mean accuracy, and step-time dominates the E2E budget. Ideas that also improve the other methods may achieve better E2E fidelity.
...
1. **Synthetic trace replay:** Using the per-request lifecycle data, reconstruct the sequence of steps for a complete request. Sum the predicted step times along the request's path and compare to the observed request-level metrics (E2E latency, TTFT, ITL). This does not require BLIS integration — it uses Python and the ground-truth data.
2. **Acceptance criteria (ordered by metrics priority):**
...
3. **Cancellation analysis:** Compute the autocorrelation of per-step signed errors along each request's step sequence. If errors are positively autocorrelated (consecutive steps both over- or under-predicted), they compound. If negatively autocorrelated or uncorrelated, they partially cancel.
