...
│   ├── hfconfig.go            # Model catalog location (--catalog / BLIS_CATALOG, no default, no search path — #1731) and the config.json resolution chain inside it (catalog entry, then HuggingFace fetch into that entry)
...
├── model_configs/             # A model CATALOG: one dir per model holding its HuggingFace config.json. Pass it via --catalog / BLIS_CATALOG (#1731); a model with no entry is auto-fetched into one (gitignored except */config.json)
