...
│   ├── network_topology.go    # NetworkTopology: the placement-derived inter-node interconnect topology (#1530) — PlacedGPUsPerNode plus NodesSpanned/MembersPerNode queries. Zero value = unknown = inert (no cross-node cost, INV-6). Written only by sim/cluster's placement sites; never a CLI flag.
