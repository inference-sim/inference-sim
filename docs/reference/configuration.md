...
1. If `--latency-model trained-physics` (default) or `roofline`:
   - Resolve the model config inside the catalog located by `--catalog` / `BLIS_CATALOG`: check `<catalog>/<model-short-name>/config.json`, fetch from HuggingFace into that entry on miss (set `HF_TOKEN` for gated models)
   ...
   - `--catalog` / `BLIS_CATALOG` locates the model config (required); `--hardware-config` overrides auto-resolution when explicitly set
