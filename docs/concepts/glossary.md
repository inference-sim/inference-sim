...
A routing signal computed as `QueueDepth + BatchSize + InFlightRequests`. Represents the total work visible at an instance, including requests that have been dispatched but not yet completed. Used by least-loaded routing and load-balance scoring. See [Cluster Architecture: Routing Pipeline](architecture.md#routing-pipeline).
