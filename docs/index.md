...
repository's `model_configs/` tree is one. Supply it with `--catalog <path>` or
`BLIS_CATALOG`; there is no default and no search path, so a run with neither is refused.
The examples throughout these docs omit it — export `BLIS_CATALOG` once, as above.
