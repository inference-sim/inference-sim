...
1. Checks the local cache at `~/.blis/model_configs/`
2. Fetches `config.json` from HuggingFace if not cached (supports `HF_TOKEN` for gated models)
3. Falls back to bundled `model_configs/` directory
