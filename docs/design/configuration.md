...
1. If `--alpha-coeffs` and `--beta-coeffs` are both all-zero and no roofline config is provided:
...
2. If coefficients are still all-zero but `--model-config-folder` and `--hardware-config` are provided:
...
| **ModelHardwareConfig** | `--model`, `--hardware`, `--tp`, `--vllm-version`, `--model-config-folder`, `--hardware-config` |
