...
7. **Document where it belongs — not in CLAUDE.md** — per-PR rationale goes in the commit body; durable detail goes in the relevant `docs/` guide or `--help`. Touch `CLAUDE.md` only when the change alters an operating rule an agent needs *every* session, and keep it a pointer when a canonical doc exists (see the charter at the top of CLAUDE.md). CLAUDE.md is loaded into every session, so a per-PR note there is expensive and re-accretes the changelog bloat #1818 removed.
