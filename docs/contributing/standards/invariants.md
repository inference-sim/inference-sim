...
¹ `cacheQueryFn` freshness is governed by `--cache-event-delay` (default 50ms), not `--snapshot-refresh-interval`. The "interval=0" / "interval>0" columns for this row refer to `--cache-event-delay`. When > 0, each instance's snapshot refreshes via CacheEventArrivalEvent after any step that allocates KV blocks plus CacheEventDelay. This models per-instance ZMQ event propagation from vLLM to the router's KVBlockIndex in production llm-d. BLIS models per-step-allocation events (one snapshot refresh per step that ran AllocateKVBlocks), not per-block-mutation like llm-d's ZMQ stream — a deliberate simplification that preserves sim/kv layering boundaries with negligible behavioral difference.

**Design implication:** When `--snapshot-refresh-interval > 0`, all Prometheus-sourced signals (QueueDepth, BatchSize, KVUtilization) share the same scrape interval — matching real vLLM deployments where all three are exposed via the same `/metrics` endpoint. `InFlightRequests` remains synchronous (gateway-local counter, not Prometheus-sourced). When `--cache-event-delay > 0` (default: 50ms), prefix cache query closures use event-driven stale snapshots of each instance's `HashToBlock` map, modeling per-instance ZMQ event propagation from production llm-d. Set `--cache-event-delay 0` for oracle mode (live cache state). The old flag `--cache-signal-delay` is a deprecated alias.
