...
**How to assert it (#1720).** Do not hand-roll a sum. There is one helper per observation layer, and each takes `total` and `rejected` separately so the full-pipeline clause below is asserted rather than assumed:
...
- `clusterConservationHolds` (`cmd/dp_placement_test.go`) — five-term, parsed from cluster-aggregate stdout, and **structurally stuck there**: the seven cluster-only counters live on `cluster.RawMetrics` and are not serialised into the JSON `MetricsOutput`, so no CLI-level test can assert the twelve-term form (#1746).

Two guard tests keep that arrangement honest. `TestINV1_HelperMatchesRegistry` diffs the helpers' term lists against the two equations stated above, so editing this section without editing the tests fails rather than drifting. `TestINV1_NoInlineClusterConservationSums` walks the AST of every `sim/cluster/*_test.go` and rejects a newly hand-rolled sum, resolving local variables so a sum accumulated through intermediates is still caught — that was the shape of one of the three sites that omitted a bucket.
