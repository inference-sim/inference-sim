...
**Convergence:** Zero CRITICAL and zero IMPORTANT items from any reviewer perspective in the current round. No minimum round count — convergence in Round 1 is valid if no reviewer flags any CRITICAL or IMPORTANT item. SUGGESTION-level items do not block convergence. See `docs/contributing/hypothesis.md` for reviewer prompts and perspective checklists.
