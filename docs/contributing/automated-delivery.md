...
- **The verify phase reads the issue *body* for the contracts, not the refinement digest.** Implement weighs body + authoritative comment refinements ([Step 1.5](pr-workflow.md#comments-can-refine-the-body)); verify does not, and its prompt does not route through `pr-workflow.md`. So an implementation that correctly followed an overriding refinement can be reported by verify as a contract divergence, and correct — whose work list is verify's findings — pointed at the superseded body. What bounds it: Step 1.5 requires the implementer to state which refinement was followed, in the PR body, which is what a verifier reads first; and no verdict merges anything. Closing it means giving verify the same read — the exact prompt hunk is in [the decision record](issue-comment-authority.md), unapplied for the reason in the next bullet.
