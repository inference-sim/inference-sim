...
- **A phase that fails or times out still reports.** Every phase has a reporter guarded on `always() && !success()`, and `.github/workflows/deliver-guards-selftest.yml` exercises that on real infrastructure rather than reasoning about it: it cancels a step with a step-level timeout and asserts the reporter is reached, plus asserts a `deliver:paused` exit stays quiet.
    ...
    Worth recording, because the original reasoning here was wrong. This was changed from `failure() || cancelled()` on the docs-based argument that a cancelled *step* satisfies neither. **The self-test found the old form does fire** on a step cancelled by a step-level timeout, because the job result becomes `failure`. `always() && !success()` is still correct — it is a strict superset — but the reason to prefer it is breadth, not the claim that the old guard was blind here. The silence observed on #1685 is explained by the runner being lost: no step executes then, `always()` ones included, which is why the stall sweep exists.
