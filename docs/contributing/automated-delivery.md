...
| `AGENT_VERDICT` | the `DELIVER-VERDICT: GREEN` / `NOT-GREEN` marker, required to be the last line of a comment posted by the automation itself. A GREEN whose comment is missing its methodology markers (the Q/A phase and a `Findings Summary` table) is **downgraded to NOT-GREEN** — see below |
...
**A GREEN must show the methodology ran.** The gate reads only the `DELIVER-VERDICT:` marker, so on its own it cannot tell a thorough review from one that skipped the `blis-pr-review` methodology and improvised a verdict — under context pressure the Q/A phase and the findings table are the first things dropped, yet the review still emits GREEN. So before the gate sees it, verify re-reads the winning comment's body and requires two markers that only a whole run produces: the Q/A phase (`CONFIDENT` / `FLAW_FOUND` / `CANNOT_ANSWER` answers) and a `Findings Summary` table. Missing either ⇒ `AGENT_VERDICT` is set to `NOT-GREEN`, so the delivery goes to another correction round (or to a human at the cap) rather than to merge. The check is `scripts/deliver-methodology-markers.sh` (unit-tested by `scripts/deliver_markers_test.go`); it runs in the verify step rather than the gate because the gate's contract is to map already-derived signals, not to parse raw comment text. Only a GREEN is checked — a NOT-GREEN or MISSING verdict already cannot reach `ready-for-merge`.
