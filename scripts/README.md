...
| `CATALOG` | `model_configs` | Model catalog root (one directory per model, each with `config.json`). Passed as `--catalog`; required since #1731. A model with no entry is auto-fetched from HuggingFace into it. |
...
If your model isn't in the catalog, either:
- Drop a `config.json` into `<CATALOG>/<your-model-slug>/`, or
- Leave the entry absent and let `blis` fetch it from HuggingFace at startup (it is
  written into `<CATALOG>/<your-model-slug>/`).
