...
# `writeAccess` per distinct author against the collaborator permission API. Emits a markdown digest
# on stdout, oldest first, and nothing at all when nothing qualifies.
#
...
# applicable at all. The label is not a second trust tier invented here — the workflows' marker
# readers already use exactly this `Bot` signal, and a Bot-typed comment on this repository requires
# an installed GitHub App, which an outside commenter cannot arrange.
#
